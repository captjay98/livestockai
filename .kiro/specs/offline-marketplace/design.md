...
export function useMarketplaceMutations() {
  ...
  const createListing = useMutation({
    ...
    onMutate: async (newListing) => {
      ...
      queryClient.setQueryData(['my-listings'], (old: Listing[] = []) => [
        { ...newListing, id: tempId, _isOptimistic: true, status: 'active' },
        ...old,
      ])
  ...
  const createContactRequest = useMutation({
    ...
    onMutate: async (request) => {
      ...
      await queryClient.cancelQueries({ queryKey: ['my-contact-requests'] })
      ...
      queryClient.setQueryData(
        ['my-contact-requests'],
        (old: ContactRequest[] = []) => [
          { ...request, id: tempId, status: 'pending', _isOptimistic: true },
          ...old,
        ],
      )
...
async function markExpiredListings(
  db: Kysely<Database>,
): Promise<number> // Returns count of newly expired listings
