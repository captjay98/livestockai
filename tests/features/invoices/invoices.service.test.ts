...
describe('Invoice Service', () => {
  describe('generateInvoiceNumber', () => {
    ...
    it('should handle malformed invoice numbers', () => {
      const currentYear = new Date().getFullYear()
      expect(generateInvoiceNumber('INVALID')).toBe(`INV-${currentYear}-0001`)
  ...
  describe('validateInvoiceData', () => {
    const validData: CreateInvoiceInput = {
      ...
      farmId: 'farm-1',
      items: [
        { description: 'Broiler chickens', quantity: 100, unitPrice: 5.5 },
      ],
    }
    ...
    it('should accept zero unit price (free item)', () => {
      const result = validateInvoiceData({
        ...validData,
        items: [{ description: 'Free item', quantity: 10, unitPrice: 0 }],
      })
      expect(result).toBeNull()
    })
    ...
    it('should validate multiple items and report first error', () => {
      const result = validateInvoiceData({
        ...
        items: [
          ...
          { description: 'Another invalid', quantity: -5, unitPrice: 10 },
        ],
      })
