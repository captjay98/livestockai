...
describe('Sales Service', () => {
    ...
    describe('validateSaleData', () => {
        ...
        it('should accept valid payment statuses', () => {
            ...
            expect(
                validateSaleData(
                    { ...validData, paymentStatus: 'partial' },
                    100,
                ),
            ).toBeNull()
        })
        ...
        it('should accept valid payment methods', () => {
            ...
            expect(
                validateSaleData(
                    { ...validData, paymentMethod: 'transfer' },
                    100,
                ),
            ).toBeNull()
            expect(
                validateSaleData(
                    { ...validData, paymentMethod: 'credit' },
                    100,
                ),
            ).toBeNull()
        })
        ...
        it('should accept valid unit types', () => {
            ...
            expect(
                validateSaleData({ ...validData, unitType: 'kg' }, 100),
            ).toBeNull()
            expect(
    ...
    describe('calculateNewTotalAmount', () => {
        it('should calculate total = quantity * unitPrice', () => {
            fc.assert(
                fc.property(
                    ...
                    (quantity, priceCents) => {
                        const unitPrice = priceCents / 100
                        const result = calculateNewTotalAmount(
                            quantity,
                            unitPrice,
                        )
                        const expected = (quantity * unitPrice).toFixed(2)
        ...
        it('should use same calculation as calculateSaleTotal', () => {
            expect(calculateNewTotalAmount(50, 5.5)).toBe(
                calculateSaleTotal(50, 5.5),
            )
