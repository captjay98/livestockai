...
describe('Expenses Service', () => {
    ...
    describe('buildExpensesSummary', () => {
        it('should build summary by category', () => {
            ...
            expect(result.byCategory.medicine).toEqual({
                count: 1,
                amount: 25.0,
            })
    ...
    describe('mapSortColumnToDbColumn', () => {
        it('should map valid sort columns', () => {
            ...
            expect(mapSortColumnToDbColumn('category')).toBe(
                'expenses.category',
            )
            expect(mapSortColumnToDbColumn('description')).toBe(
                'expenses.description',
            )
            expect(mapSortColumnToDbColumn('supplierName')).toBe(
                'suppliers.name',
            )
        ...
        it('should default to date for invalid columns', () => {
            expect(mapSortColumnToDbColumn('invalid' as any)).toBe(
                'expenses.date',
            )
    ...
    describe('shouldUpdateFeedInventory', () => {
        ...
        it('should return false when feed quantity is missing', () => {
            const result = shouldUpdateFeedInventory(
                'feed',
                'starter',
                undefined,
            )
        ...
        it('should return false for zero or negative quantity', () => {
            ...
            expect(shouldUpdateFeedInventory('feed', 'starter', -10)).toBe(
                false,
            )
        })
        ...
        it('should handle all feed types', () => {
            ...
            for (const feedType of feedTypes) {
                expect(shouldUpdateFeedInventory('feed', feedType, 50)).toBe(
                    true,
                )
            }
        })
