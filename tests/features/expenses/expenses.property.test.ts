...
describe('expenses aggregation logic', () => {
    ...
    function calculateSummary(results: Array<{ category: string; count: string | number; totalAmount: string }>) {
            ...
            fc.property(
                fc.array(
                    fc.record({
                        ...
                        totalAmount: fc.float({ min: 0, max: 1000000, noNaN: true }).map(n => n.toFixed(2))
                    }),
                    { minLength: 0, maxLength: 20 }
                ),
                (rows) => {
                    // Filter to unique categories for the summary logic test to match typical DB group by
                    const uniqueRows = Array.from(new Map(rows.map(r => [r.category, r])).values())
                    ...
                    uniqueRows.forEach(row => {
                        expect(result.byCategory[row.category].amount).toBe(parseFloat(row.totalAmount))
                        expect(result.byCategory[row.category].count).toBe(Number(row.count))
