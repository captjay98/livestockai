...
describe('Settings Service', () => {
    describe('validateSettingData', () => {
        ...
        it('should reject invalid currency decimals', () => {
            ...
            expect(
                validateSettingData('currencyDecimals', 'invalid' as any),
            ).toBe('Currency decimals must be between 0 and 4')
        })

        it('should accept valid currency symbol position', () => {
            expect(
                validateSettingData('currencySymbolPosition', 'before'),
            ).toBeNull()
            expect(
                validateSettingData('currencySymbolPosition', 'after'),
            ).toBeNull()
        })

        it('should reject invalid currency symbol position', () => {
            expect(
                validateSettingData('currencySymbolPosition', 'invalid'),
            ).toBe('Currency symbol position must be "before" or "after"')
        })
        ...
        it('should accept valid first day of week', () => {
            fc.assert(
                fc.property(fc.integer({ min: 0, max: 6 }), (day) => {
                    expect(
                        validateSettingData('firstDayOfWeek', day),
                    ).toBeNull()
                }),
                { numRuns: 10 },
            )
        ...
        it('should reject invalid alert thresholds', () => {
            expect(
                validateSettingData('lowStockThresholdPercent', 0),
            ).not.toBeNull()
            expect(
                validateSettingData('lowStockThresholdPercent', 101),
            ).not.toBeNull()
            expect(
                validateSettingData('mortalityAlertPercent', 0),
            ).not.toBeNull()
            expect(
                validateSettingData('mortalityAlertQuantity', 0),
            ).not.toBeNull()
        })
        ...
        it('should accept valid fiscal year settings', () => {
            fc.assert(
                fc.property(fc.integer({ min: 1, max: 12 }), (month) => {
                    expect(
                        validateSettingData('fiscalYearStartMonth', month),
                    ).toBeNull()
                }),
                { numRuns: 12 },
            )
        ...
        it('should reject invalid fiscal year month', () => {
            expect(
                validateSettingData('fiscalYearStartMonth', 0),
            ).not.toBeNull()
            expect(
                validateSettingData('fiscalYearStartMonth', 13),
            ).not.toBeNull()
        })
    })
    ...
    describe('parseSettingValue', () => {
        ...
        it('should parse boolean values in nested objects', () => {
            expect(parseSettingValue('notifications.lowStock', 'true')).toBe(
                true,
            )
            ...
            expect(parseSettingValue('notifications.lowStock', 'false')).toBe(
                true,
            )
