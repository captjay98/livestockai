    ...
    describe('Unit Tests', () => {
        ...
        describe('parseCurrency', () => {
            it('parses valid currency strings', () => {
                ...
                expect(parseCurrency('₦ 2,500,000.00')?.toString()).toBe(
                    '2500000',
                )
            })
        ...
        describe('sumAmounts', () => {
            it('sums multiple amounts', () => {
                ...
                expect(
                    sumAmounts('100.50', '200.25', '300.25').toString(),
                ).toBe('601')
            })
        ...
        it('toDbString and toDecimal are inverse operations', () => {
            fc.assert(
                fc.property(currencyArb, (amount) => {
                    ...
                    expect(backToDecimal.toFixed(2)).toBe(
                        toDecimal(amount).toFixed(2),
                    )
                }),
                { numRuns: 100 },
            )
        ...
        it('sumAmounts is associative and commutative', () => {
            fc.assert(
                fc.property(
                    ...
                    (a, b, c) => {
                        // Commutative: a + b = b + a
                        expect(sumAmounts(a, b).equals(sumAmounts(b, a))).toBe(
                            true,
                        )

                        // Associative: (a + b) + c = a + (b + c)
                        const leftAssoc = sumAmounts(
                            sumAmounts(a, b).toNumber(),
                            c,
                        )
                        const rightAssoc = sumAmounts(
                            a,
                            sumAmounts(b, c).toNumber(),
                        )
