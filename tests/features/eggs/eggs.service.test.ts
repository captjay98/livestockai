...
describe('Eggs Service', () => {
  describe('validateEggCollectionData', () => {
    ...
    it('should reject empty batch ID', () => {
      const result = validateEggCollectionData({ ...validData, batchId: '' })
      ...
      it('should never have negative inventory', () => {
        fc.assert(
          fc.property(
            ...
            (records) => {
              const totals = calculateEggTotals(records)
              expect(totals.currentInventory).toBeGreaterThanOrEqual(0)
            },
          ),
          { numRuns: 100 },
        )
        ...
        fc.assert(
          fc.property(
            ...
            (collected1, broken1, sold1) => {
              ...
              expect(totals2.totalCollected).toBe(totals1.totalCollected * 2)
              expect(totals2.totalBroken).toBe(totals1.totalBroken * 2)
              expect(totals2.totalSold).toBe(totals1.totalSold * 2)
            },
          ),
          { numRuns: 50 },
        )
