...
describe('Water Quality Service', () => {
    describe('validateReadingData', () => {
        ...
        it('should accept edge case ammonia values', () => {
            expect(
                validateReadingData({ ...validData, ammoniaMgL: 0 }),
            ).toBeNull()
            expect(
                validateReadingData({ ...validData, ammoniaMgL: 0.02 }),
            ).toBeNull()
        })
    })
    ...
    describe('determineParameterStatus', () => {
        ...
        describe('dissolved oxygen status', () => {
            ...
            it('should return warning for slightly below minimum', () => {
                expect(
                    determineParameterStatus('dissolvedOxygenMgL', 3.5),
                ).toBe('warning')
            })
        ...
        describe('ammonia status', () => {
            it('should return optimal for very low ammonia', () => {
                expect(determineParameterStatus('ammoniaMgL', 0.005)).toBe(
                    'optimal',
                )
                expect(determineParameterStatus('ammoniaMgL', 0.01)).toBe(
                    'optimal',
                )
            })

            it('should return acceptable for threshold ammonia', () => {
                expect(determineParameterStatus('ammoniaMgL', 0.02)).toBe(
                    'acceptable',
                )
            })

            it('should return warning for slightly above threshold', () => {
                expect(determineParameterStatus('ammoniaMgL', 0.03)).toBe(
                    'warning',
                )
            })

            it('should return critical for high ammonia', () => {
                expect(determineParameterStatus('ammoniaMgL', 0.1)).toBe(
                    'critical',
                )
            })
        })
    ...
    describe('calculateParameterTrend', () => {
        ...
        it('should detect improving dissolved oxygen', () => {
            ...
            const result = calculateParameterTrend(
                readings,
                'dissolvedOxygenMgL',
            )
                    ...
                    fc.property(
                        fc.array(
                            fc.record({
                                ...
                                temperatureCelsius: fc.float({
                                    min: -10,
                                    max: 50,
                                }),
                                dissolvedOxygenMgL: fc.float({
                                    min: 0,
                                    max: 20,
                                }),
