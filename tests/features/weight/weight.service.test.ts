...
describe('Weight Service', () => {
    ...
    describe('getExpectedAdg', () => {
        it('should return correct ADG for known species', () => {
            expect(getExpectedAdg('broiler')).toBe(
                EXPECTED_ADG_BY_SPECIES.broiler,
            )
            expect(getExpectedAdg('layer')).toBe(EXPECTED_ADG_BY_SPECIES.layer)
            expect(getExpectedAdg('catfish')).toBe(
                EXPECTED_ADG_BY_SPECIES.catfish,
            )
            expect(getExpectedAdg('tilapia')).toBe(
                EXPECTED_ADG_BY_SPECIES.tilapia,
            )
        })
                ...
                fc.assert(
                    fc.property(
                        ...
                        (values) => {
                            ...
                            const expected =
                                values.reduce((sum, v) => sum + v / 100, 0) /
                                values.length
                            expect(calculateAverageWeight(records)).toBe(
                                expected,
                            )
                        },
                    ),
