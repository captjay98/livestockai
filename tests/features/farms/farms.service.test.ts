...
describe('Farm Service', () => {
    ...
    describe('validateFarmData', () => {
        ...
        it('should reject invalid farm type', () => {
            expect(
                validateFarmData({ ...validData, type: 'invalid' as any }),
        ...
        it('should reject missing location', () => {
            expect(
                validateFarmData({ name: 'Farm', type: 'poultry' } as any),
            ).toBe('Farm location is required')
        })
            ...
            fc.assert(
                fc.property(
                    ...
                    (isOwner, otherOwnersCount) => {
                        const result = canRemoveUserFromFarm(
                            isOwner,
                            otherOwnersCount,
                        )
                        const expectedCanRemove = !(
                            isOwner && otherOwnersCount === 0
                        )
                        ...
                        fc.property(
                            ...
                            (otherOwnersCount) => {
                                ...
                                const isOwnerDemotion =
                                    currentRole === 'owner' &&
                                    newRole !== 'owner'
                                const expectedCanChange =
                                    !isOwnerDemotion || otherOwnersCount > 0
