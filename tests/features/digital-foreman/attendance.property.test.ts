...
describe('Attendance Service - Property Tests', () => {
  describe('calculateHoursWorked', () => {
    ...
    it('should return non-negative hours when checkout is after checkin', () => {
      fc.assert(
        fc.property(validDate, fc.integer({ min: 1, max: 24 * 60 }), (checkIn, minutesLater) => {
          const checkOut = new Date(checkIn.getTime() + minutesLater * 60 * 1000)
    ...
    it('should calculate correct hours for known durations', () => {
      ...
      // 1 hour
      expect(calculateHoursWorked(checkIn, new Date('2024-01-01T10:00:00'))).toBe(1)
      
      // 8 hours
      expect(calculateHoursWorked(checkIn, new Date('2024-01-01T17:00:00'))).toBe(8)
      
      // 30 minutes = 0.5 hours
      expect(calculateHoursWorked(checkIn, new Date('2024-01-01T09:30:00'))).toBe(0.5)
    })
      ...
      fc.assert(
        fc.property(
          ...
          (checkIn, hours1, hours2) => {
            const checkOut1 = new Date(checkIn.getTime() + hours1 * 60 * 60 * 1000)
            const checkOut2 = new Date(checkIn.getTime() + hours2 * 60 * 60 * 1000)
  ...
  describe('isDuplicateCheckIn', () => {
    ...
    it('should detect duplicates within default 5 minute threshold', () => {
      fc.assert(
        fc.property(validDate, fc.integer({ min: 0, max: 4 }), (lastCheckIn, minutesLater) => {
          const newCheckIn = new Date(lastCheckIn.getTime() + minutesLater * 60 * 1000)
    ...
    it('should not detect duplicates beyond threshold', () => {
      fc.assert(
        fc.property(validDate, fc.integer({ min: 6, max: 60 }), (lastCheckIn, minutesLater) => {
          const newCheckIn = new Date(lastCheckIn.getTime() + minutesLater * 60 * 1000)
    ...
    it('should be symmetric (order of times should not matter for detection)', () => {
      fc.assert(
        fc.property(validDate, fc.integer({ min: 0, max: 10 }), (time1, minutesDiff) => {
  ...
  describe('shouldAutoCheckOut', () => {
    it('should return false for same day', () => {
      fc.assert(
        fc.property(validDate, fc.integer({ min: 0, max: 23 }), (date, hour) => {
    ...
    it('should return true for different days', () => {
      fc.assert(
        fc.property(validDate, fc.integer({ min: 1, max: 30 }), (checkIn, daysLater) => {
          const current = new Date(checkIn)
  ...
  describe('calculateAttendanceSummary', () => {
    ...
    it('should return zeros for empty array', () => {
      const summary = calculateAttendanceSummary([], new Date('2024-01-01'), new Date('2024-01-31'))
    ...
    it('should count flagged check-ins correctly', () => {
      const records: Array<AttendanceRecord> = [
        { checkIn: new Date('2024-01-15T09:00:00'), checkOut: new Date('2024-01-15T17:00:00') },
    ...
    it('should count unique work days', () => {
      const records: Array<AttendanceRecord> = [
        { checkIn: new Date('2024-01-15T09:00:00'), checkOut: new Date('2024-01-15T12:00:00') },
        { checkIn: new Date('2024-01-15T13:00:00'), checkOut: new Date('2024-01-15T17:00:00') }, // same day
        { checkIn: new Date('2024-01-16T09:00:00'), checkOut: new Date('2024-01-16T17:00:00') },
      ]
    ...
    it('should filter records by period', () => {
      const records: Array<AttendanceRecord> = [
        { checkIn: new Date('2024-01-05T09:00:00'), checkOut: new Date('2024-01-05T17:00:00') }, // before period
        { checkIn: new Date('2024-01-15T09:00:00'), checkOut: new Date('2024-01-15T17:00:00') }, // in period
        { checkIn: new Date('2024-02-05T09:00:00'), checkOut: new Date('2024-02-05T17:00:00') }, // after period
      ]
