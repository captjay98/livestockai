...
describe('Payroll Service - Property Tests', () => {
        ...
        it('should return full rate for full month', () => {
            fc.assert(
                fc.property(
                    ...
                    positiveNumber,
                    (totalDays, rate) => {
                        const wages = calculateMonthlyWages(
                            totalDays,
                            totalDays,
                            rate,
                        )
            ...
            fc.assert(
                fc.property(
                    ...
                    (totalDays, rate) => {
                        ...
                        expect(wages).toBeCloseTo(
                            (halfDays / totalDays) * rate,
                            2,
                        )
                    },
                ),
        ...
        it('should return zero for zero days worked', () => {
            fc.assert(
                fc.property(
                    ...
                    positiveNumber,
                    (totalDays, rate) => {
                        expect(calculateMonthlyWages(0, totalDays, rate)).toBe(
                            0,
                        )
                    },
                ),
    ...
    describe('calculateGrossWages', () => {
        it('should dispatch to correct calculation based on rate type', () => {
            const hourlyConfig: WageConfig = {
                rateAmount: 15,
                rateType: 'hourly',
                currency: 'USD',
            }
            const dailyConfig: WageConfig = {
                rateAmount: 100,
                rateType: 'daily',
                currency: 'USD',
            }
            const monthlyConfig: WageConfig = {
                rateAmount: 3000,
                rateType: 'monthly',
                currency: 'USD',
            }
    ...
    describe('calculateDaysWorked', () => {
        ...
        it('should count each unique date once regardless of check-in count', () => {
            fc.assert(
                fc.property(
                    fc.array(fc.integer({ min: 1, max: 28 }), {
                        minLength: 1,
                        maxLength: 50,
                    }),
                    (days) => {
                        const checkIns = days.map((day) => ({
                            date: new Date(
                                `2024-01-${day.toString().padStart(2, '0')}T09:00:00`,
                            ),
                        }))
