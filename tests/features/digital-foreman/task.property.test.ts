...
// Arbitraries
const uuid = fc.uuid()
const validStatus = fc.constantFrom<AssignmentStatus>(
    'pending',
    'in_progress',
    'completed',
    'pending_approval',
...
const completableStatus = fc.constantFrom<AssignmentStatus>(
    'pending',
    'in_progress',
)
...
describe('Task Service - Property Tests', () => {
    describe('validateTaskCompletion', () => {
        ...
        it('should reject when worker is not assignee', () => {
            fc.assert(
                fc.property(
                    ...
                    completableStatus,
                    fc.boolean(),
                    (assigneeId, attempterId, status, requiresPhoto) => {
        ...
        it('should reject non-completable statuses', () => {
            const nonCompletableStatus = fc.constantFrom<AssignmentStatus>(
                'completed',
                'pending_approval',
                'verified',
                'rejected',
            ...
            fc.assert(
                fc.property(
                    ...
                    (workerId, status, currentTime) => {
                        ...
                        expect(isTaskOverdue(assignment, currentTime)).toBe(
                            false,
                        )
                    },
                ),
        ...
        it('should return true when current time is past due date', () => {
            fc.assert(
                fc.property(
                    ...
                    (workerId, status, dueDate, daysLater) => {
        ...
        it('should return false when current time is before due date', () => {
            fc.assert(
                fc.property(
                    ...
                    validDate,
                    fc.integer({ min: 1, max: 30 }),
                    (workerId, status, dueDate, daysBefore) => {
                        const currentTime = new Date(dueDate)
    ...
    describe('calculateTaskMetrics', () => {
        ...
        it('should count total correctly', () => {
            fc.assert(
                fc.property(
                    fc.array(assignmentArb, { minLength: 1, maxLength: 50 }),
                    (assignments) => {
            ...
            fc.assert(
                fc.property(
                    ...
                    (assignments) => {
                        ...
                        const actualCompleted = assignments.filter((a) =>
                            completedStatuses.includes(a.status),
                        ).length
                        const actualPending = assignments.filter((a) =>
