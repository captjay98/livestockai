...
function validateIngestion(input: IngestInput, now: Date = new Date()): ValidationResult {
  ...
  if (input.recordedAt > now) {
    return { valid: false, error: 'Reading timestamp cannot be in the future' }
  }

  // Check timestamp is not too old
  const ageHours = (now.getTime() - input.recordedAt.getTime()) / (1000 * 60 * 60)
...
function checkRateLimit(state: RateLimitState, now: Date = new Date()): { allowed: boolean; newState: RateLimitState } {
      ...
      fc.assert(
        fc.property(
          ...
          (value, minutesAgo, sensorId) => {
            ...
            const result = validateIngestion({ value, recordedAt, sensorId }, now)
      ...
      fc.assert(
        fc.property(
          ...
          (value, minutesInFuture, sensorId) => {
            const recordedAt = new Date(now.getTime() + minutesInFuture * 60 * 1000)
  ...
  describe('Property 3: Old timestamps are rejected', () => {
    ...
    it('should reject readings older than the maximum age', () => {
      ...
      fc.assert(
        fc.property(
          fc.float({ min: -1000, max: 1000, noNaN: true, noDefaultInfinity: true }),
          fc.integer({ min: MAX_READING_AGE_HOURS * 60 + 1, max: 10000 }), // minutes ago (too old)
          fc.uuid(),
          (value, minutesAgo, sensorId) => {
            const recordedAt = new Date(now.getTime() - minutesAgo * 60 * 1000)
      ...
      fc.assert(
        fc.property(
          ...
          (requestCount) => {
            ...
            expect(result.newState.requestCount).toBe(requestCount + 1)
          }
        ),
        { numRuns: 100 }
      )
    ...
    it('should reset rate limit after window expires', () => {
      fc.assert(
        fc.property(
          ...
          fc.integer({ min: RATE_LIMIT_WINDOW_MS, max: RATE_LIMIT_WINDOW_MS * 10 }),
          (requestCount, windowAgeMs) => {
            const now = new Date()
            const windowStart = new Date(now.getTime() - windowAgeMs)
            const state: RateLimitState = { requestCount, windowStart }
