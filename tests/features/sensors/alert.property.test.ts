...
): { shouldAlert: boolean; severity: 'warning' | 'critical' | null; type: string | null } {
  // Critical thresholds take precedence
  if (thresholds.maxValue !== null && value > thresholds.maxValue) {
    return { shouldAlert: true, severity: 'critical', type: 'threshold_high' }
  }
  if (thresholds.minValue !== null && value < thresholds.minValue) {
    return { shouldAlert: true, severity: 'critical', type: 'threshold_low' }
  }
  
  // Warning thresholds
  if (thresholds.warningMaxValue !== null && value > thresholds.warningMaxValue) {
    return { shouldAlert: true, severity: 'warning', type: 'threshold_high' }
  }
  if (thresholds.warningMinValue !== null && value < thresholds.warningMinValue) {
      ...
      fc.assert(
        fc.property(
          ...
          (warningMax, criticalMax, excess) => {
            ...
            const actualWarningMax = Math.min(warningMax, criticalMax - 1)
      ...
      fc.assert(
        fc.property(
          ...
          (minValue, maxValue, testHigh) => {
            ...
            expect(result.shouldAlert).toBe(true)
            expect(result.type).toBe(testHigh ? 'threshold_high' : 'threshold_low')
          }
        ),
        { numRuns: 100 }
      )
