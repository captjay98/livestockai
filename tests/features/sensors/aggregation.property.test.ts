    ...
    it('should calculate correct average', () => {
      fc.assert(
        fc.property(
          ...
          (values) => {
            const readings = values.map(v => ({ value: v, recordedAt: new Date() }))
      ...
      fc.assert(
        fc.property(
          ...
          (values) => {
            ...
            expect(result).not.toBeNull()
            if (result) {
              expect(result.avgValue).toBeGreaterThanOrEqual(result.minValue)
              expect(result.avgValue).toBeLessThanOrEqual(result.maxValue)
            }
          }
        ),
        { numRuns: 100 }
