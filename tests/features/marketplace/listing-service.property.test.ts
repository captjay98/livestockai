...
describe('listing-service property tests', () => {
  it('Property 1: Required Fields Validation', () => {
    const validListing = fc.record({
      species: fc.constantFrom('poultry', 'fish', 'cattle', 'goats', 'sheep'),
    ...
    const requiredFields = ['species', 'quantity', 'minPrice', 'maxPrice', 'location'] as const
    ...
    fc.assert(
      fc.property(validListing, fc.constantFrom(...requiredFields), (listing, fieldToRemove) => {
  ...
  it('Property 17: Expiration Warning Detection', () => {
    ...
    fc.assert(
      fc.property(
        fc.date({ min: new Date(threeDaysFromNow.getTime() + 24 * 60 * 60 * 1000) }),
        (dateBeyond3Days) => {
