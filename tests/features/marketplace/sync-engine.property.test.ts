      ...
      fc.assert(
        fc.property(
          ...
          (objA, objB) => {
            ...
            if (objA.updatedAt.getTime() > objB.updatedAt.getTime()) {
              expect(result).toBe(objA)
            } else if (objB.updatedAt.getTime() > objA.updatedAt.getTime()) {
    ...
    it('should be symmetric - same winner regardless of argument order', () => {
      fc.assert(
        fc.property(
          ...
          (objA, objB) => {
            // Skip when timestamps are equal (tie-breaker is not symmetric by design)
            fc.pre(objA.updatedAt.getTime() !== objB.updatedAt.getTime())

            const resultAB = resolveConflict(objA, objB)
            const resultBA = resolveConflict(objB, objA)

            // Both should return the same winner (by timestamp comparison)
            expect(resultAB.updatedAt.getTime()).toBe(
              resultBA.updatedAt.getTime(),
            )
          },
        ),
        { numRuns: 100 },
