...
describe('Onboarding Types', () => {
  ...
  it('should have abbreviated steps as subset of full steps', () => {
    fc.assert(
      fc.property(
        fc.constant({ full: ONBOARDING_STEPS, abbrev: ABBREVIATED_STEPS }),
        ({ full, abbrev }) => {
    ...
    fc.assert(
      fc.property(
        ...
        (stepIndex) => {
          ...
          expect(progress.completedSteps).not.toContain(progress.currentStep)
          // Completed steps should be in order
          for (let i = 0; i < progress.completedSteps.length; i++) {
            expect(ONBOARDING_STEPS.indexOf(progress.completedSteps[i])).toBe(i)
          }
          return true
        },
      ),
