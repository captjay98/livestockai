            ...
            fc.assert(
                fc.property(
                    ...
                    (weight1, weight2, daysDiff) => {
                        ...
                        const expectedADG =
                            ((weight2 - weight1) * 1000) / daysDiff
                        expect(result.adgGramsPerDay).toBeCloseTo(
                            expectedADG,
                            2,
                        )
                ...
                fc.property(
                    ...
                    (weight, days) => {
                        ...
                        const sampleDate = new Date(
                            acquisitionDate.getTime() +
                                days * 24 * 60 * 60 * 1000,
                        )

                        const samples = [
                            { averageWeightKg: weight, date: sampleDate },
                        ]
            ...
            fc.assert(
                fc.property(
                    ...
                    (weight1, weight2, days) => {
                        const date1 = new Date('2024-01-01')
                        const date2 = new Date(
                            date1.getTime() + days * 24 * 60 * 60 * 1000,
                        )
            ...
            fc.assert(
                fc.property(
                    ...
                    (actual, expected) => {
                        ...
                        const result = calculateDeviationPercent(
                            actual,
                            expected,
                        )
