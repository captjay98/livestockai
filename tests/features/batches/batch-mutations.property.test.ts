...
describe('Batch Mutation Optimistic Updates - Property Tests', () => {
    ...
    const createBatchInputArb = fc.record({
        ...
        sourceSize: fc.option(
            fc.constantFrom('day-old', 'fingerling', 'jumbo'),
            {
                nil: undefined,
            },
        ),
        ...
        it('should generate batch temp IDs with correct prefix', () => {
            fc.assert(
                fc.property(fc.constant(null), () => {
                    ...
                    expect(tempId.startsWith(`${TEMP_ID_PREFIX}batch-`)).toBe(
                        true,
                    )
            ...
            fc.assert(
                fc.property(createBatchInputArb, (input) => {
                    ...
                    const optimisticBatch: Omit<BatchRecord, 'id'> = {
                        ...
                        totalCost: (
                            input.initialQuantity * input.costPerUnit
                        ).toString(),
                        status: 'active',
                        ...
                        targetPricePerUnit:
                            input.targetPricePerUnit?.toString() || null,
                    ...
                    expect(result[0].initialQuantity).toBe(
                ...
                fc.property(
                    ...
                    (batches, newStatus) => {
                        ...
                        const targetIndex = Math.floor(
                            Math.random() * batches.length,
                        )
                        const targetId = batches[targetIndex].id

                        const result = updateById<BatchRecord>(
                            batches,
                            targetId,
                            {
                                status: newStatus,
                        ...
                        const updatedBatch = result.find(
                            (b) => b.id === targetId,
                        )
            ...
            fc.assert(
                fc.property(
                    ...
                    (existingBatches, serverBatch) => {
                        ...
                        expect(
                            result.find((b) => b.id === tempId),
                        ).toBeUndefined()

                        // Server batch should exist
                        const found = result.find(
                            (b) => b.id === serverBatch.id,
                        )
            ...
            fc.assert(
                fc.property(
                    ...
                    (existingBatches, serverBatch) => {
                        ...
                        expect(
                            result.find((b) => b.id === serverBatch.id),
                        ).toBeDefined()
                    },
                ),
                { numRuns: 100 },
