...
describe('batches/server logic', () => {
    const livestockTypes: Array<LivestockType> = [
        'poultry',
        'fish',
        'cattle',
        'goats',
        ...
        it('should match MODULE_METADATA source size options', () => {
            livestockTypes.forEach((type) => {
                ...
                const moduleEntry = Object.entries(MODULE_METADATA).find(
                    ([_, metadata]) => metadata.livestockTypes.includes(type),
                )
    ...
    describe('SOURCE_SIZE_OPTIONS', () => {
        ...
        it('should match getSourceSizeOptions output', () => {
            livestockTypes.forEach((type) => {
                expect(SOURCE_SIZE_OPTIONS[type]).toEqual(
                    getSourceSizeOptions(type),
                )
            })
        })
    })
                ...
                fc.property(
                    ...
                    (initial, mortalities, sales) => {
                        ...
                        const actualSales = Math.min(
                            sales,
                            initial - actualMortalities,
                        )

                        const current =
                            initial - actualMortalities - actualSales
