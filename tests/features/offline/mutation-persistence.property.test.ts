        ...
        it('should preserve numeric data fields through serialization', () => {
            fc.assert(
                fc.property(serializedMutationArb, (mutation) => {
                    ...
                    expect(deserialized.data.quantity).toBe(
                        mutation.data.quantity,
                    )
        ...
        it('should preserve date data fields through serialization', () => {
            fc.assert(
                fc.property(serializedMutationArb, (mutation) => {
                    ...
                    expect(deserializedDate.getTime()).toBe(
        ...
        it('should be idempotent - sorting twice gives same result', () => {
            fc.assert(
                fc.property(mutationListArb, (mutations) => {
                    ...
                    expect(sorted2.map((m) => m.id)).toEqual(
                        sorted1.map((m) => m.id),
                    )
                }),
                { numRuns: 100 },
                ...
                fc.property(
                    ...
                    (mutations, results) => {
                        ...
                        mutations.forEach((original) => {
                            const syncedMutation = synced.find(
                                (m) => m.id === original.id,
                            )
                            expect(syncedMutation).toBeDefined()
                            expect(syncedMutation!.type).toBe(original.type)
                            expect(syncedMutation!.entityType).toBe(
                                original.entityType,
                            )
                            expect(syncedMutation!.entityId).toBe(
                                original.entityId,
                            )
                            expect(syncedMutation!.timestamp).toBe(
                                original.timestamp,
                            )
                        })
                    },
