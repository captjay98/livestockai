...
describe('Pending Count Accuracy - Property Tests', () => {
    ...
    function calculatePendingCount(
        mutations: Array<SimulatedMutation>,
    ): number {
        return mutations.filter((m) => m.status === 'pending' || m.isPaused)
            .length
    }
    ...
    describe('Property 7: Pending Count Accuracy', () => {
        describe('Pending count calculation', () => {
            it('should count all pending mutations correctly', () => {
                fc.assert(
                    fc.property(mutationListArb, (mutations) => {
                        ...
                        expect(actualPendingMutations.length).toBe(
            ...
            it('should count all paused mutations correctly', () => {
                fc.assert(
                    fc.property(mutationListArb, (mutations) => {
                        const expectedPausedCount =
                            calculatePausedCount(mutations)

                        // Verify paused count matches filter
                        const actualPausedMutations = mutations.filter(
                            (m) => m.isPaused,
                        )

                        expect(actualPausedMutations.length).toBe(
                            expectedPausedCount,
                        )
                    }),
                    { numRuns: 100 },
                ...
                fc.assert(
                    fc.property(mutationListArb, (mutations) => {
                        const expectedFailedCount =
                            calculateFailedCount(mutations)
                        ...
                        expect(actualFailedMutations.length).toBe(
                            expectedFailedCount,
                        )
                    }),
                    { numRuns: 100 },
                ...
                fc.assert(
                    fc.property(
                        ...
                        (mutations) => {
                            ...
                            expect(pausedPendingCount).toBeLessThanOrEqual(
                                pendingCount,
                            )
                        },
                    ),
                ...
                fc.assert(
                    fc.property(
                        ...
                        (existingMutations, newKey) => {
                            const initialCount =
                                calculatePendingCount(existingMutations)
                            ...
                            const newCount =
                                calculatePendingCount(updatedMutations)
                ...
                fc.assert(
                    fc.property(
                        mutationListArb.filter((m) =>
                            m.some(
                                (mut) =>
                                    mut.status === 'pending' && !mut.isPaused,
                            ),
                        ),
                        (mutations) => {
                            const initialCount =
                                calculatePendingCount(mutations)
                    ...
                    fc.property(
                        ...
                        (mutations) => {
                            ...
                            const newFailedCount =
                                calculateFailedCount(updatedMutations)

                            expect(newPendingCount).toBe(
            ...
            it('displayed count should match actual mutation count', () => {
                fc.assert(
                    fc.property(mutationListArb, (mutations) => {
                        ...
                        const displayedPausedCount =
                            pausedMutations.filter(Boolean).length
                        ...
                        expect(displayedFailedCount).toBe(
                            mutations.filter((m) => m.status === 'error')
                                .length,
                        )
                    }),
                    { numRuns: 100 },
            ...
            it('total pending should equal pending + paused when counting for display', () => {
                fc.assert(
                    fc.property(mutationListArb, (mutations) => {
                        ...
                        const pausedCount = mutations.filter(
                            (m) => m.isPaused,
                        ).length
