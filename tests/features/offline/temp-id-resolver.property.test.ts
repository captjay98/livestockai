        ...
        fc.asyncProperty(
          ...
          async (pairs) => {
            ...
            for (let i = 0; i < tempIds.length; i++) {
              expect(tempIdResolver.isResolved(tempIds[i])).toBe(true)
              expect(tempIdResolver.resolve(tempIds[i])).toBe(pairs[i][1])
            }
      ...
      await fc.assert(
        fc.asyncProperty(
          ...
          async (resolvedType, unresolvedType, serverId) => {
            const resolvedTempId = generateEntityTempId(resolvedType)
            const unresolvedTempId = generateEntityTempId(unresolvedType)
