            ...
            fc.assert(
                fc.property(
                    ...
                    (revenue, expenses) => {
                        ...
                        ).toBeCloseTo(Math.round(expected * 10) / 10)
                    },
                ),
                { numRuns: 100 },
            )
            ...
            fc.assert(
                fc.property(
                    ...
                    (initial, current) => {
                        ...
                        expect(
                            calculateMortalityRate(initial, current),
                        ).toBeCloseTo(Math.round(expected * 10) / 10)
                    },
                ),
                { numRuns: 100 },
            ...
            fc.assert(
                fc.property(
                    ...
                    (eggs, birds, days) => {
                        const expected =
                            birds > 0 && days > 0
                                ? Math.round((eggs / (birds * days)) * 1000) /
                                  10
                                : 0
                        expect(
                            calculateLayingPercentage(eggs, birds, days),
                        ).toBe(expected)
                    },
                ),
                { numRuns: 100 },
