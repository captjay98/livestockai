      ...
      fc.assert(
        fc.property(
          ...
          (revenue, expensePercent) => {
            const expenses = Math.floor((revenue * expensePercent) / 100)
            const profit = calculateProfit(revenue, expenses)
      ...
      fc.assert(
        fc.property(
          ...
          (transactions) => {
            ...
            const totalProfit = calculateProfit(totalRevenue, totalExpenses)

            const sumOfProfits = transactions.reduce(
              (sum, t) => sum + calculateProfit(t.revenue, t.expenses),
              0,
            )

            expect(Math.abs(totalProfit - sumOfProfits)).toBeLessThan(0.01)
          },
        ),
        { numRuns: 100 },
      )
