        ...
        fc.assert(
            fc.property(moduleSetArb, (enabledModules) => {
                ...
                filtered.forEach((item) => {
                    ...
                    const isExpectedModule = expectedModuleItems.includes(
                        item.name,
                    )
    ...
    it('Property 5: Filtering is idempotent', () => {
        fc.assert(
            fc.property(moduleSetArb, (enabledModules) => {
                ...
                const filtered1 = filterNavigationByModules(
                    testNav,
                    enabledModules,
                )
                const filtered2 = filterNavigationByModules(
                    testNav,
                    enabledModules,
                )
    ...
    it('Property 6: Subset of modules shows subset of navigation items', () => {
        fc.assert(
            fc.property(moduleSetArb, moduleSetArb, (modules1, modules2) => {
                ...
                const filteredSubset = filterNavigationByModules(
                    testNav,
                    subset,
                )
                const filteredAll = filterNavigationByModules(
                    testNav,
                    allModules,
                )
                ...
                expect(filteredSubset.length).toBeLessThanOrEqual(
                    filteredAll.length,
                )
            }),
            { numRuns: 100 },
    ...
    it('Property 7: Filtering preserves relative order of items', () => {
        fc.assert(
            fc.property(moduleSetArb, (enabledModules) => {
                ...
                const testNav = [...coreItems, ...moduleItems].filter(
                    (item) => {
                ...
                if (filtered.length > 1) {
                    ...
                    for (let i = 1; i < originalIndices.length; i++) {
                        expect(originalIndices[i]).toBeGreaterThan(
                            originalIndices[i - 1],
                        )
                    }
                }
