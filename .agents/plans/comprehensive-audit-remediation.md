...
if (!ADMIN_PASSWORD) {
  ...
  console.error('   Set it before running: ADMIN_PASSWORD=your-secure-password bun run db:seed')
  process.exit(1)
}
```
...
export const FEED_SORT_COLUMNS = ['date', 'cost', 'quantityKg', 'feedType', 'createdAt'] as const
export type FeedSortColumn = typeof FEED_SORT_COLUMNS[number]

// Weight samples allowed columns
export const WEIGHT_SORT_COLUMNS = ['date', 'averageWeightKg', 'sampleSize', 'createdAt'] as const
export type WeightSortColumn = typeof WEIGHT_SORT_COLUMNS[number]

// Water quality allowed columns
export const WATER_QUALITY_SORT_COLUMNS = ['date', 'ph', 'temperatureCelsius', 'dissolvedOxygenMgL', 'ammoniaMgL', 'createdAt'] as const
export type WaterQualitySortColumn = typeof WATER_QUALITY_SORT_COLUMNS[number]

// Mortality records allowed columns
export const MORTALITY_SORT_COLUMNS = ['date', 'quantity', 'cause', 'createdAt'] as const
export type MortalitySortColumn = typeof MORTALITY_SORT_COLUMNS[number]

// Customers allowed columns
export const CUSTOMER_SORT_COLUMNS = ['name', 'phone', 'email', 'location', 'customerType', 'createdAt'] as const
export type CustomerSortColumn = typeof CUSTOMER_SORT_COLUMNS[number]

// Suppliers allowed columns
export const SUPPLIER_SORT_COLUMNS = ['name', 'phone', 'email', 'location', 'supplierType', 'createdAt'] as const
export type SupplierSortColumn = typeof SUPPLIER_SORT_COLUMNS[number]

// Eggs allowed columns
export const EGG_SORT_COLUMNS = ['date', 'quantityCollected', 'quantityBroken', 'quantitySold', 'createdAt'] as const
export type EggSortColumn = typeof EGG_SORT_COLUMNS[number]

// Invoices allowed columns
export const INVOICE_SORT_COLUMNS = ['date', 'dueDate', 'totalAmount', 'status', 'invoiceNumber', 'createdAt'] as const
export type InvoiceSortColumn = typeof INVOICE_SORT_COLUMNS[number]

// Vaccinations allowed columns
export const VACCINATION_SORT_COLUMNS = ['dateAdministered', 'vaccineName', 'nextDueDate', 'createdAt'] as const
...
export const validateWeightSort = createSortValidator(WEIGHT_SORT_COLUMNS, 'date')
export const validateWaterQualitySort = createSortValidator(WATER_QUALITY_SORT_COLUMNS, 'date')
export const validateMortalitySort = createSortValidator(MORTALITY_SORT_COLUMNS, 'date')
export const validateCustomerSort = createSortValidator(CUSTOMER_SORT_COLUMNS, 'createdAt')
export const validateSupplierSort = createSortValidator(SUPPLIER_SORT_COLUMNS, 'createdAt')
export const validateEggSort = createSortValidator(EGG_SORT_COLUMNS, 'date')
export const validateInvoiceSort = createSortValidator(INVOICE_SORT_COLUMNS, 'date')
export const validateVaccinationSort = createSortValidator(VACCINATION_SORT_COLUMNS, 'dateAdministered')
```
...
dataQuery = dataQuery.orderBy(sql.raw(`feed_records."${safeSortColumn}"`), order)
```
...
import { validateFeedSort, FEED_SORT_COLUMNS } from '~/lib/validation/sort-columns'
...
- [ ] Task 1: vite.config.ts secrets removed
- [ ] Task 2: auth/config.ts hardened
- [ ] Task 3: production.ts requires password
- [ ] Task 4: FCR calculation fixed
- [ ] Task 5: Performance indexes added to migration
- [ ] Task 6: Sort column validators created
- [ ] Task 7: feed/repository.ts updated
- [ ] Task 8: All 8 remaining repositories updated

### Phase 2 Tasks (HIGH)
- [ ] Task 9: Farm detail route refactored
- [ ] Task 10: Type drift fixed (59 interfaces)
- [ ] Task 11: Circular dependency decoupled
- [ ] Task 12: Water quality thresholds added
- [ ] Task 13: Growth standards completed
- [ ] Task 14: Error swallowing fixed
- [ ] Task 15: Dashboard N+1 consolidated
- [ ] Task 16: All 14 bloated routes refactored

### Phase 3 Tasks (MEDIUM)
- [ ] Task 17: React performance optimizations
- [ ] Task 18: Monitoring service tests
- [ ] Task 19: Missing livestock KPIs
- [ ] Task 20: Vaccination schedules
- [ ] Task 21: TanStack Query caching
