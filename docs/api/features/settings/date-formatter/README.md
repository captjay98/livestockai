[**OpenLivestock API Reference**](../../../README.md)

***

[OpenLivestock API Reference](../../../modules.md) / features/settings/date-formatter

# features/settings/date-formatter

## Functions

- [formatDate](functions/formatDate.md)
- [formatDateTime](functions/formatDateTime.md)
- [formatLongDate](functions/formatLongDate.md)
- [formatRelativeDate](functions/formatRelativeDate.md)
- [formatShortDate](functions/formatShortDate.md)
- [formatTime](functions/formatTime.md)
- [getDateFormatPattern](functions/getDateFormatPattern.md)
