[**LivestockAI API Reference**](../../../../README.md)

---

[LivestockAI API Reference](../../../../modules.md) / [features/feed/server](../README.md) / FeedRecord

# Interface: FeedRecord

Defined in: features/feed/server.ts:9

Core interface representing a single feeding record with joined batch and supplier info

## Properties

### batchId

> **batchId**: `string`

Defined in: features/feed/server.ts:11

---

### batchSpecies

> **batchSpecies**: `string` \| `null`

Defined in: features/feed/server.ts:12

---

### brandName

> **brandName**: `string` \| `null`

Defined in: features/feed/server.ts:14

---

### cost

> **cost**: `string`

Defined in: features/feed/server.ts:16

---

### date

> **date**: `Date`

Defined in: features/feed/server.ts:17

---

### feedType

> **feedType**: `string`

Defined in: features/feed/server.ts:13

---

### id

> **id**: `string`

Defined in: features/feed/server.ts:10

---

### notes

> **notes**: `string` \| `null`

Defined in: features/feed/server.ts:19

---

### quantityKg

> **quantityKg**: `string`

Defined in: features/feed/server.ts:15

---

### supplierName

> **supplierName**: `string` \| `null`

Defined in: features/feed/server.ts:18
