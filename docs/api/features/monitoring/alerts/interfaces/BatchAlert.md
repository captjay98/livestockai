[**LivestockAI API Reference**](../../../../README.md)

---

[LivestockAI API Reference](../../../../modules.md) / [features/monitoring/alerts](../README.md) / BatchAlert

# Interface: BatchAlert

Defined in: features/monitoring/alerts.ts:13

## Properties

### batchId

> **batchId**: `string`

Defined in: features/monitoring/alerts.ts:15

---

### id

> **id**: `string`

Defined in: features/monitoring/alerts.ts:14

---

### message

> **message**: `string`

Defined in: features/monitoring/alerts.ts:19

---

### metadata?

> `optional` **metadata**: `Record`\<`string`, `any`\>

Defined in: features/monitoring/alerts.ts:22

---

### source

> **source**: [`AlertSource`](../type-aliases/AlertSource.md)

Defined in: features/monitoring/alerts.ts:18

---

### species

> **species**: `string`

Defined in: features/monitoring/alerts.ts:16

---

### timestamp

> **timestamp**: `Date`

Defined in: features/monitoring/alerts.ts:20

---

### type

> **type**: [`AlertType`](../type-aliases/AlertType.md)

Defined in: features/monitoring/alerts.ts:17

---

### value?

> `optional` **value**: `number`

Defined in: features/monitoring/alerts.ts:21
