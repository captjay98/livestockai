[**LivestockAI API Reference**](../../../../README.md)

---

[LivestockAI API Reference](../../../../modules.md) / [lib/db/types](../README.md) / MedicationInventoryTable

# Interface: MedicationInventoryTable

Defined in: lib/db/types.ts:471

## Properties

### expiryDate

> **expiryDate**: `Date` \| `null`

Defined in: lib/db/types.ts:477

---

### farmId

> **farmId**: `string`

Defined in: lib/db/types.ts:473

---

### id

> **id**: `Generated`\<`string`\>

Defined in: lib/db/types.ts:472

---

### medicationName

> **medicationName**: `string`

Defined in: lib/db/types.ts:474

---

### minThreshold

> **minThreshold**: `number`

Defined in: lib/db/types.ts:478

---

### quantity

> **quantity**: `number`

Defined in: lib/db/types.ts:475

---

### unit

> **unit**: `"kg"` \| `"liter"` \| `"vial"` \| `"bottle"` \| `"sachet"` \| `"ml"` \| `"g"` \| `"tablet"`

Defined in: lib/db/types.ts:476

---

### updatedAt

> **updatedAt**: `Generated`\<`Date`\>

Defined in: lib/db/types.ts:479
