    ...
    fc.assert(
      fc.property(
        ...
        (eggsCollected, currentQuantity) => {
          const percentage = calculateLayingPercentage(eggsCollected, currentQuantity)
  ...
  it('typical layer hen laying percentage is 70-95%', () => {
    ...
    fc.assert(
      fc.property(
        ...
        fc.double({ min: 0.70, max: 0.95, noNaN: true }), // laying rate
        (flockSize, layingRate) => {
          const eggsCollected = Math.floor(flockSize * layingRate)
          const percentage = calculateLayingPercentage(eggsCollected, flockSize)
    ...
    fc.assert(
      fc.property(
        ...
        (collected, rawSold, rawBroken) => {
          const { sold, broken } = constrainQuantities(collected, rawSold, rawBroken)
          const inventory = calculateEggInventory(collected, sold, broken)
      ...
      fc.property(
        ...
        (records) => {
          ...
          for (const record of records) {
            ...
            const available = totalCollected - totalSold - totalBroken
          ...
          const inventory = calculateEggInventory(totalCollected, totalSold, totalBroken)
          
          expect(inventory).toBe(totalCollected - totalSold - totalBroken)
          expect(inventory).toBeGreaterThanOrEqual(0)
        }
      ),
      { numRuns: 100 }
    ...
    fc.assert(
      fc.property(
        ...
        (collected, soldAmount, brokenAmount) => {
          const initialInventory = calculateEggInventory(collected, 0, 0)
          const afterSale = calculateEggInventory(collected, soldAmount, 0)
          const afterBroken = calculateEggInventory(collected, 0, brokenAmount)
