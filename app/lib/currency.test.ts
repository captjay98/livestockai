...
describe('Currency Utilities', () => {
  describe('Unit Tests', () => {
    ...
    describe('formatNaira', () => {
      ...
      it('formats large amounts with separators', () => {
        expect(formatNaira(1500000)).toBe('₦1,500,000.00')
        expect(formatNaira('2500000.75')).toBe('₦2,500,000.75')
      })

      it('handles Decimal input', () => {
        expect(formatNaira(new Decimal('1500.50'))).toBe('₦1,500.50')
      })
    })

    describe('parseNaira', () => {
      it('parses valid naira strings', () => {
        expect(parseNaira('1500.50')?.toString()).toBe('1500.5')
        expect(parseNaira('₦1,500.50')?.toString()).toBe('1500.5')
        expect(parseNaira('₦ 2,500,000.00')?.toString()).toBe('2500000')
      })

      it('returns null for invalid strings', () => {
        expect(parseNaira('invalid')).toBeNull()
    ...
    describe('formatNairaCompact', () => {
      it('formats millions correctly', () => {
        expect(formatNairaCompact(1500000)).toBe('₦1.5M')
        expect(formatNairaCompact(2300000)).toBe('₦2.3M')
      })

      it('formats thousands correctly', () => {
        expect(formatNairaCompact(1500)).toBe('₦1.5K')
        expect(formatNairaCompact(25000)).toBe('₦25.0K')
      })

      it('formats small amounts normally', () => {
        expect(formatNairaCompact(500)).toBe('₦500.00')
        expect(formatNairaCompact(999)).toBe('₦999.00')
      })
    })
  ...
  describe('Property Tests', () => {
    ...
    it('parseNaira and formatNaira are consistent', () => {
      fc.assert(
        fc.property(currencyArb, (amount) => {
          const formatted = formatNaira(amount)
          const parsed = parseNaira(formatted)
