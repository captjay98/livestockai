...
export async function up(db: Kysely<any>): Promise<void> {
  ...
  await sql`ALTER TABLE users ADD CONSTRAINT users_role_check CHECK (role IN ('admin', 'staff'))`.execute(
    db,
  )
  ...
  await sql`ALTER TABLE farms ADD CONSTRAINT farms_type_check CHECK (type IN ('poultry', 'fishery', 'mixed'))`.execute(
    db,
  )
  ...
  await sql`ALTER TABLE batches ADD CONSTRAINT batches_livestock_type_check CHECK ("livestockType" IN ('poultry', 'fish'))`.execute(
    db,
  )
  ...
  await sql`ALTER TABLE batches ADD CONSTRAINT batches_cost_per_unit_check CHECK ("costPerUnit" >= 0)`.execute(
    db,
  )
  await sql`ALTER TABLE batches ADD CONSTRAINT batches_total_cost_check CHECK ("totalCost" >= 0)`.execute(
    db,
  )
  ...
  await sql`ALTER TABLE feed_records ADD CONSTRAINT feed_records_feed_type_check CHECK ("feedType" IN ('starter', 'grower', 'finisher', 'layer_mash', 'fish_feed'))`.execute(
    db,
  )
  await sql`ALTER TABLE feed_records ADD CONSTRAINT feed_records_quantity_kg_check CHECK ("quantityKg" > 0)`.execute(
    db,
  )
  await sql`ALTER TABLE feed_records ADD CONSTRAINT feed_records_cost_check CHECK (cost >= 0)`.execute(
    db,
  )
  ...
  await sql`ALTER TABLE egg_records ADD CONSTRAINT egg_records_quantity_collected_check CHECK ("quantityCollected" >= 0)`.execute(
    db,
  )
  await sql`ALTER TABLE egg_records ADD CONSTRAINT egg_records_quantity_broken_check CHECK ("quantityBroken" >= 0)`.execute(
    db,
  )
  await sql`ALTER TABLE egg_records ADD CONSTRAINT egg_records_quantity_sold_check CHECK ("quantitySold" >= 0)`.execute(
    db,
  )
  ...
  await sql`ALTER TABLE weight_samples ADD CONSTRAINT weight_samples_sample_size_check CHECK ("sampleSize" > 0)`.execute(
    db,
  )
  await sql`ALTER TABLE weight_samples ADD CONSTRAINT weight_samples_average_weight_kg_check CHECK ("averageWeightKg" > 0)`.execute(
    db,
  )
  ...
  await sql`ALTER TABLE treatments ADD CONSTRAINT treatments_withdrawal_days_check CHECK ("withdrawalDays" >= 0)`.execute(
    db,
  )
  ...
  await sql`ALTER TABLE water_quality ADD CONSTRAINT water_quality_ph_check CHECK (ph >= 0 AND ph <= 14)`.execute(
    db,
  )
  await sql`ALTER TABLE water_quality ADD CONSTRAINT water_quality_dissolved_oxygen_check CHECK ("dissolvedOxygenMgL" >= 0)`.execute(
    db,
  )
  await sql`ALTER TABLE water_quality ADD CONSTRAINT water_quality_ammonia_check CHECK ("ammoniaMgL" >= 0)`.execute(
    db,
  )
  ...
  await sql`ALTER TABLE sales ADD CONSTRAINT sales_livestock_type_check CHECK ("livestockType" IN ('poultry', 'fish', 'eggs'))`.execute(
    db,
  )
  await sql`ALTER TABLE sales ADD CONSTRAINT sales_quantity_check CHECK (quantity > 0)`.execute(
    db,
  )
  await sql`ALTER TABLE sales ADD CONSTRAINT sales_unit_price_check CHECK ("unitPrice" >= 0)`.execute(
    db,
  )
  await sql`ALTER TABLE sales ADD CONSTRAINT sales_total_amount_check CHECK ("totalAmount" >= 0)`.execute(
    db,
  )
  ...
  await sql`ALTER TABLE expenses ADD CONSTRAINT expenses_category_check CHECK (category IN ('feed', 'medicine', 'equipment', 'utilities', 'labor', 'transport', 'other'))`.execute(
    db,
  )
  await sql`ALTER TABLE expenses ADD CONSTRAINT expenses_amount_check CHECK (amount >= 0)`.execute(
    db,
  )
  ...
  await sql`ALTER TABLE invoices ADD CONSTRAINT invoices_total_amount_check CHECK ("totalAmount" >= 0)`.execute(
    db,
  )
  ...
  await sql`ALTER TABLE invoice_items ADD CONSTRAINT invoice_items_quantity_check CHECK (quantity > 0)`.execute(
    db,
  )
  await sql`ALTER TABLE invoice_items ADD CONSTRAINT invoice_items_unit_price_check CHECK ("unitPrice" >= 0)`.execute(
    db,
  )
  await sql`ALTER TABLE invoice_items ADD CONSTRAINT invoice_items_total_check CHECK (total >= 0)`.execute(
    db,
  )
  ...
  await db.schema
    ...
    .execute()
  await db.schema
    .createIndex('idx_mortality_records_date')
    .on('mortality_records')
    .column('date')
    .execute()
  await db.schema
    ...
    .execute()
  await db.schema
    .createIndex('idx_feed_records_date')
    .on('feed_records')
    .column('date')
    .execute()
  await db.schema
    .createIndex('idx_egg_records_batch_id')
    .on('egg_records')
    .column('batchId')
    .execute()
  await db.schema
    .createIndex('idx_egg_records_date')
    .on('egg_records')
    .column('date')
    .execute()
  await db.schema
    .createIndex('idx_weight_samples_batch_id')
    .on('weight_samples')
    .column('batchId')
    .execute()
  await db.schema
    .createIndex('idx_weight_samples_date')
    .on('weight_samples')
    .column('date')
    .execute()
  await db.schema
    .createIndex('idx_vaccinations_batch_id')
    .on('vaccinations')
    .column('batchId')
    .execute()
  await db.schema
    .createIndex('idx_vaccinations_next_due_date')
    .on('vaccinations')
    .column('nextDueDate')
    .execute()
  await db.schema
    .createIndex('idx_treatments_batch_id')
    .on('treatments')
    .column('batchId')
    .execute()
  await db.schema
    .createIndex('idx_water_quality_batch_id')
    .on('water_quality')
    .column('batchId')
    .execute()
  await db.schema
    .createIndex('idx_water_quality_date')
    .on('water_quality')
    .column('date')
    .execute()
  await db.schema
  ...
  await db.schema
    ...
    .execute()
  await db.schema
    .createIndex('idx_sales_customer_id')
    .on('sales')
    .column('customerId')
    .execute()
  await db.schema
    .createIndex('idx_sales_date')
    .on('sales')
    .column('date')
    .execute()
  await db.schema
    .createIndex('idx_expenses_farm_id')
    .on('expenses')
    .column('farmId')
    .execute()
  await db.schema
  ...
  await db.schema
    ...
    .execute()
  await db.schema
    .createIndex('idx_invoices_customer_id')
    .on('invoices')
    .column('customerId')
    .execute()
  await db.schema
    .createIndex('idx_invoices_farm_id')
    .on('invoices')
    .column('farmId')
    .execute()
  await db.schema
    .createIndex('idx_invoices_date')
    .on('invoices')
    .column('date')
    .execute()
  await db.schema
