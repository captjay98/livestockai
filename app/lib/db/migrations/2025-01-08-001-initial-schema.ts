...
export async function up(db: Kysely<any>): Promise<void> {
  ...
  await sql`
    CREATE TABLE formulation_usage (
      ...
      "formulationId" UUID NOT NULL REFERENCES saved_formulations(id) ON DELETE CASCADE,
      "batchId" UUID REFERENCES batches(id) ON DELETE SET NULL,
      "userId" UUID NOT NULL,
      "usedAt" TIMESTAMPTZ NOT NULL DEFAULT NOW(),
      "batchSizeKg" DECIMAL(10,2) NOT NULL,
  ...
  await sql`CREATE INDEX idx_formulation_usage_formulation_id ON formulation_usage("formulationId")`.execute(
    db,
  )
  await sql`CREATE INDEX idx_formulation_usage_batch_id ON formulation_usage("batchId")`.execute(
    db,
  )
  await sql`CREATE INDEX idx_formulation_usage_user_id ON formulation_usage("userId")`.execute(
    db,
  )
  await sql`CREATE INDEX idx_formulation_usage_used_at ON formulation_usage("usedAt" DESC)`.execute(
    db,
  )
  ...
  `.execute(db)
  await sql`CREATE INDEX idx_worker_profiles_user_id ON worker_profiles ("userId")`.execute(
    db,
  )
  await sql`CREATE INDEX idx_worker_profiles_farm_id ON worker_profiles ("farmId")`.execute(
    db,
  )
  await sql`CREATE INDEX idx_worker_profiles_employment_status ON worker_profiles ("employmentStatus")`.execute(
    db,
  )
  ...
  `.execute(db)
  await sql`CREATE INDEX idx_worker_check_ins_worker_id ON worker_check_ins ("workerId")`.execute(
    db,
  )
  await sql`CREATE INDEX idx_worker_check_ins_farm_id ON worker_check_ins ("farmId")`.execute(
    db,
  )
  await sql`CREATE INDEX idx_worker_check_ins_check_in_time ON worker_check_ins ("checkInTime")`.execute(
    db,
  )
  await sql`CREATE INDEX idx_worker_check_ins_open ON worker_check_ins ("workerId", "farmId") WHERE "checkOutTime" IS NULL`.execute(
    db,
  )
  ...
  `.execute(db)
  await sql`CREATE INDEX idx_task_assignments_worker_id ON task_assignments ("workerId")`.execute(
    db,
  )
  await sql`CREATE INDEX idx_task_assignments_farm_id ON task_assignments ("farmId")`.execute(
    db,
  )
  await sql`CREATE INDEX idx_task_assignments_status ON task_assignments (status)`.execute(
    db,
  )
  await sql`CREATE INDEX idx_task_assignments_due_date ON task_assignments ("dueDate")`.execute(
    db,
  )
  await sql`CREATE INDEX idx_task_assignments_pending_approval ON task_assignments ("farmId") WHERE status = 'pending_approval'`.execute(
    db,
  )
  ...
  `.execute(db)
  await sql`CREATE INDEX idx_task_photos_assignment_id ON task_photos ("assignmentId")`.execute(
    db,
  )
  ...
  `.execute(db)
  await sql`CREATE INDEX idx_wage_payments_worker_id ON wage_payments ("workerId")`.execute(
    db,
  )
  await sql`CREATE INDEX idx_wage_payments_period_id ON wage_payments ("payrollPeriodId")`.execute(
    db,
  )
  await sql`CREATE INDEX idx_wage_payments_farm_id ON wage_payments ("farmId")`.execute(
    db,
  )
  ...
  await db.schema
    ...
    .addColumn('contactCount', 'integer', (col) => col.notNull().defaultTo(0))
    .addColumn('createdAt', 'timestamptz', (col) =>
      col.notNull().defaultTo(sql`now()`),
    )
    .addColumn('updatedAt', 'timestamptz', (col) =>
  ...
  await db.schema
    ...
    .addColumn('buyerId', 'uuid', (col) => col.notNull().references('users.id'))
    .addColumn('message', 'text', (col) => col.notNull())
  ...
  await db.schema
    ...
    .column('viewedAt')
    .execute()
  // Composite indexes for deduplication queries (date filtering done in app layer)
  await db.schema
    .createIndex('idx_listing_views_user_listing')
    .on('listing_views')
    .columns(['listingId', 'viewerId'])
    .execute()
  await db.schema
    .createIndex('idx_listing_views_ip_listing')
    .on('listing_views')
    .columns(['listingId', 'viewerIp'])
    .execute()
  ...
  await db.schema
    ...
    .addColumn('code', 'varchar(2)', (col) => col.notNull().unique())
    .addColumn('name', 'varchar(100)', (col) => col.notNull())
  ...
  await db.schema
    ...
    .addColumn('countryId', 'uuid', (col) =>
      col.notNull().references('countries.id'),
    )
  ...
  await db.schema
    ...
    .column('parentId')
    .execute()
  await sql`CREATE INDEX idx_regions_active ON regions("isActive") WHERE "isActive" = true`.execute(
    db,
  )
  ...
  await db.schema
    ...
    .addColumn('assignedAt', 'timestamp', (col) => col.defaultTo(sql`NOW()`))
  ...
  await db.schema
    ...
    .addColumn('purpose', 'text', (col) => col.notNull())
    .addColumn('requestedDurationDays', 'integer', (col) => col.defaultTo(90))
    ...
    .addCheckConstraint(
      ...
      sql`"requestedDurationDays" BETWEEN 30 AND 365`,
    )
    .execute()
  ...
  await db.schema
    ...
    .execute()
  await sql`CREATE INDEX idx_access_requests_pending ON access_requests(status, "expiresAt") WHERE status = 'pending'`.execute(
    db,
  )
  ...
  await db.schema
    ...
    .addColumn('financialVisibility', 'boolean', (col) => col.defaultTo(false))
  ...
  await sql`CREATE UNIQUE INDEX idx_active_access_grant ON access_grants("userId", "farmId") WHERE "revokedAt" IS NULL`.execute(
    db,
  )
  await db.schema
    ...
    .execute()
  await sql`CREATE INDEX idx_access_grants_expiring ON access_grants("expiresAt") WHERE "revokedAt" IS NULL`.execute(
    db,
  )
  await db.schema
  ...
  await db.schema
    ...
    .execute()
  await sql`CREATE INDEX idx_farms_district ON farms("districtId") WHERE "districtId" IS NOT NULL`.execute(
    db,
  )
  ...
  await sql`
        ...
        BEGIN
          IF NEW."districtId" IS NOT NULL THEN
            IF NOT EXISTS (SELECT 1 FROM regions WHERE id = NEW."districtId" AND level = 2) THEN
  ...
  await sql`
        CREATE TRIGGER check_farm_district
          BEFORE INSERT OR UPDATE OF "districtId" ON farms
  ...
  await sql`
        ...
        BEGIN
          IF OLD."districtId" IS DISTINCT FROM NEW."districtId" THEN
            ...
            SET "revokedAt" = NOW(),
                "revokedReason" = 'Farm changed district'
            WHERE "farmId" = NEW.id
  ...
  await sql`
        CREATE TRIGGER revoke_grants_on_district_change
          AFTER UPDATE OF "districtId" ON farms
  ...
  await sql`ALTER TABLE outbreak_alerts ADD CONSTRAINT outbreak_alerts_severity_check CHECK (severity IN ('watch', 'alert', 'critical'))`.execute(
    db,
  )
  await sql`ALTER TABLE outbreak_alerts ADD CONSTRAINT outbreak_alerts_status_check CHECK (status IN ('active', 'monitoring', 'resolved', 'false_positive'))`.execute(
    db,
  )
  ...
  await db.schema
    ...
    .addColumn('agentId', 'uuid', (col) => col.notNull().references('users.id'))
    .addColumn('farmId', 'uuid', (col) => col.notNull().references('farms.id'))
    ...
    .addColumn('farmerAcknowledged', 'boolean', (col) => col.defaultTo(false))
  ...
  await sql`ALTER TABLE visit_records ADD CONSTRAINT visit_records_visit_type_check CHECK ("visitType" IN ('routine', 'emergency', 'follow_up'))`.execute(
    db,
  )
