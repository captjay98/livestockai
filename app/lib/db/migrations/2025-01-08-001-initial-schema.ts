...
export async function up(db: Kysely<any>): Promise<void> {
  ...
  await db.schema
    ...
    .addUniqueConstraint('growth_standards_species_day_unique', ['species', 'day'])
    .execute()
  ...
  await db.schema.createIndex('audit_logs_entity_idx').on('audit_logs').columns(['entityType', 'entityId']).execute()
  await db.schema.createIndex('audit_logs_user_idx').on('audit_logs').column('userId').execute()
...
export async function down(db: Kysely<any>): Promise<void> {
  ...
  // Drop all tables
  const tables = [
    'growth_standards', 'market_prices', 'audit_logs',
    'invoice_items', 'invoices', 'expenses', 'sales',
