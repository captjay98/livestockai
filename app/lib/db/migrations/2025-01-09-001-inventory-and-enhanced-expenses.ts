...
export async function up(db: Kysely<any>): Promise<void> {
  ...
  await sql`ALTER TABLE feed_inventory ADD CONSTRAINT feed_inventory_feed_type_check CHECK ("feedType" IN ('starter', 'grower', 'finisher', 'layer_mash', 'fish_feed'))`.execute(
    db,
  )

  // Create index
  await db.schema
    .createIndex('idx_feed_inventory_farm_id')
    .on('feed_inventory')
    .column('farmId')
    .execute()
  await db.schema
    .createIndex('idx_feed_inventory_farm_type')
    .on('feed_inventory')
    .columns(['farmId', 'feedType'])
    .execute()
}
