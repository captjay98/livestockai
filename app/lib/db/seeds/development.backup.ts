 ...
 *
 * For production (minimal data), use: bun run db:seed
 */
...
// Dynamic date calculation - all dates relative to "today"
...
export async function seedDev() {
    ...
    try {
        ...
        const tables = [
            'invoice_items',
            'sales', // Must delete before invoices (FK constraint)
            'invoices',
            'water_quality',
            'weight_samples',
            'treatments',
        ...
        // Create Default Admin using Better Auth-compatible helper
        ...
        await db
            ...
            .values({ userId: result.userId, farmId: farm.id, role: 'owner' })
            .execute()

        // Enable farm modules (poultry + aquaculture for mixed farm)
        await db
            .insertInto('farm_modules')
            .values([
                { farmId: farm.id, moduleKey: 'poultry', enabled: true },
                { farmId: farm.id, moduleKey: 'aquaculture', enabled: true },
            ])
            .execute()
        console.log(`✅ ${farm.name} (modules: poultry, aquaculture)\n`)
        ...
        const vetSupplier = suppliers.find((s) =>
            s.products.includes('vaccines'),
        )!
        ...
        // Batch 1: SOLD - Started ~7 months ago, sold at 5 weeks - Normal price
        const batch1Start = subtractMonths(7)
        const batch1Size = randomInt(
            BROILER.batchSize.min,
            BROILER.batchSize.max,
        )
        ...
        const b1Week1Deaths = Math.floor(
            batch1Size * BROILER.mortalityRate.week1,
        )
        ...
        // Batch 2: SOLD - Started ~5 months ago, sold at 8 weeks - Normal price
        const batch2Start = subtractMonths(5)
        const batch2Size = randomInt(
            BROILER.batchSize.min,
            BROILER.batchSize.max,
        )
        ...
        // Batch 3: ACTIVE - Started ~6 weeks ago - PREMIUM price (scarce month)
        const batch3Start = subtractDays(42)
        const batch3Size = randomInt(
            BROILER.batchSize.min,
            BROILER.batchSize.max,
        )
        ...
        // Fish Batch 1: SOLD - Started ~7 months ago, harvested ~2.5 months ago (4.5 months growth)
        const fishBatch1Start = subtractMonths(7)
        ...
        // Fish Batch 2: ACTIVE - Started ~4 months ago, currently growing
        const fishBatch2Start = subtractMonths(4)
        ...
        // Monthly utilities (last 8 months)
        for (let i = 7; i >= 0; i--) {
            const utilityDate = subtractMonths(i)
            utilityDate.setDate(25) // Set to 25th of each month
            expenses.push({
                farmId: farm.id,
                batchId: null,
                category: 'utilities',
        ...
        expenses.push(
            {
                ...
                category: 'equipment',
                amount: '45000.00',
                date: subtractMonths(7), // When farm started
            ...
            },
            {
                ...
                category: 'equipment',
                amount: '25000.00',
                date: subtractMonths(4), // Mid-way through
        ...
        const totalBroilerRevenue =
            b1Alive * BROILER.salePrice5Weeks +
            b2Alive * BROILER.salePrice8Weeks
        ...
        console.log('\n💰 Revenue Generated:')
        console.log(
            `   • Broiler Sales: ₦${totalBroilerRevenue.toLocaleString()}`,
        )
