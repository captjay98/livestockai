    ...
    fc.assert(
      fc.property(
        ...
        (records, targetFarmId) => {
          ...
          const otherFarmRecords = records.filter(r => r.farmId !== targetFarmId)
      ...
      fc.property(
        ...
        (batches, records, targetFarmId) => {
          ...
          const recordsWithValidBatches = records.map((r, i) => ({
            ...r,
            batchId: batches.length > 0 ? batches[i % batches.length].id : r.batchId
          }))
    ...
    fc.assert(
      fc.property(
        ...
        (records) => {
          ...
          const expectedCount = records.filter(r => r.farmId === targetFarmId).length
