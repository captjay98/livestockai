    ...
    fc.assert(
      fc.property(
        ...
        (initialQuantity, mortalityRecords) => {
          ...
          let totalDeaths = mortalityRecords.reduce((sum, q) => sum + q, 0)
    ...
    fc.assert(
      fc.property(
        ...
        (initialQuantity, deaths1Pct, deaths2Pct) => {
          ...
          const deaths1 = Math.floor((deaths1Pct / 100) * initialQuantity)
          const deaths2 = Math.floor((deaths2Pct / 100) * initialQuantity)

          const rate1 = calculateMortalityRate(initialQuantity, deaths1)
          const rate2 = calculateMortalityRate(initialQuantity, deaths2)
