...
describe('Property 15: Customer Revenue Aggregation', () => {
  ...
  const saleAmountArb = fc.double({ min: 100, max: 10000000, noNaN: true })
      ...
      fc.property(
        ...
        (sales, customerId) => {
          ...
          const salesWithoutCustomer = sales.map(s => ({
            ...s,
            customerId: s.customerId === customerId ? `other-${s.customerId}` : s.customerId
          }))
          
          const totalSpent = calculateCustomerTotalSpent(salesWithoutCustomer, customerId)
  ...
  it('total_spent increases with each sale', () => {
    fc.assert(
      fc.property(
        ...
        (customerId, amounts) => {
          let runningTotal = 0
          const sales: Array<{ customerId: string; totalAmount: number }> = []
      ...
      fc.property(
        ...
        (sales) => {
          ...
          const totalFromCustomers = customerIds.reduce(
            (sum, customerId) => sum + calculateCustomerTotalSpent(sales, customerId),
            0
          )
          
          const totalFromSales = sales.reduce((sum, s) => sum + s.totalAmount, 0)
  ...
  it('customer with single large sale ranks higher than multiple small sales', () => {
    fc.assert(
      fc.property(
        ...
        fc.array(fc.double({ min: 100, max: 500, noNaN: true }), { minLength: 1, maxLength: 10 }), // small sales
        (customerA, customerB, largeSale, smallSales) => {
          ...
          const sales = [
            ...
            ...smallSales.map(amount => ({ customerId: customerB, totalAmount: amount }))
          ]
