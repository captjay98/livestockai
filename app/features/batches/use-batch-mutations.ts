    ...
    >({
        ...
        onMutate: async ({ batch }) => {
            ...
            const optimisticBatch: Omit<BatchRecord, 'id'> = {
                ...
                totalCost: (
                    batch.initialQuantity * batch.costPerUnit
                ).toString(),
                status: 'active',
                ...
                targetPricePerUnit:
                    batch.targetPricePerUnit?.toString() || null,
        ...
        onSettled: () => {
            ...
            queryClient.invalidateQueries({ queryKey: ['farm-modules'] })
        },
    })
    ...
    >({
        ...
        onError: (error, variables, context) => {
            ...
            if (isConflictError(error)) {
                ...
                if (conflictData) {
                    const { serverVersion, clientVersion, resolution } =
                        conflictData
