...
export function calculateADG(
    ...
    if (samples.length >= 2) {
        ...
        const weightDiffG =
            (recent.averageWeightKg - previous.averageWeightKg) * 1000
    ...
    if (samples.length === 1) {
        ...
        const daysSinceAcquisition = differenceInDays(
            sample.date,
            acquisitionDate,
        )
...
export function calculateExpectedADG(
    ...
    if (before) {
        ...
        if (beforeBefore) {
            const weightDiff =
                before.expected_weight_g - beforeBefore.expected_weight_g
    ...
    if (after) {
        const afterAfterArr = growthStandards
            ...
            .sort((a, b) => a.day - b.day)
        const afterAfter =
            afterAfterArr.length > 0 ? afterAfterArr[0] : undefined

        if (afterAfter) {
            const weightDiff =
                afterAfter.expected_weight_g - after.expected_weight_g
...
export function generateChartData(
    ...
    for (const standard of growthStandards) {
        ...
        const sample = weightSamples.find(
            (s) => differenceInDays(s.date, sampleDate) === 0,
        )
