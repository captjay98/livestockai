...
const saveGeofenceSchema = z.object({
  ...
  vertices: z.array(z.object({ lat: z.number(), lng: z.number() })).max(20).optional(),
...
export const createPayrollPeriodFn = createServerFn({ method: 'POST' })
  ...
  .handler(async ({ data }) => {
    ...
    const { insertPayrollPeriod, getOverlappingPeriods } = await import('./repository')
    const { validatePayrollPeriod } = await import('./payroll-service')

    const existing = await getOverlappingPeriods(db, data.farmId, data.startDate, data.endDate)
    const validation = validatePayrollPeriod(data.startDate, data.endDate, existing)
...
export const getPayrollSummaryFn = createServerFn({ method: 'GET' })
  ...
  .handler(async ({ data }) => {
    ...
    const { getDb } = await import('~/lib/db')
    const db = await getDb()
    const { getWorkersByFarm, getCheckInsByWorker, getPaymentsByPeriod } = await import('./repository')
    const { calculateGrossWages, calculateOutstandingBalance } = await import('./payroll-service')

    const period = await db.selectFrom('payroll_periods').selectAll().where('id', '=', data.payrollPeriodId).executeTakeFirst()
    if (!period) throw new AppError('PAYROLL_PERIOD_NOT_FOUND')

    const farm = await db.selectFrom('farms').select(['name']).where('id', '=', data.farmId).executeTakeFirst()
    const workers = await getWorkersByFarm(db, data.farmId)
    const payments = await getPaymentsByPeriod(db, data.payrollPeriodId)

    const workerSummaries = await Promise.all(workers.map(async (w) => {
      const checkIns = await getCheckInsByWorker(db, w.id, period.startDate, period.endDate)
      const totalHours = checkIns.reduce((sum, c) => sum + Number(c.hoursWorked || 0), 0)
      const grossWages = calculateGrossWages(totalHours, { rateAmount: Number(w.wageRateAmount), rateType: w.wageRateType, currency: 'USD' })
      const paid = payments.filter(p => p.workerName === w.userName).reduce((sum, p) => sum + Number(p.amount), 0)
      return { 
        ...w, 
        totalHours, 
        grossWages, 
    ...
    return { period, workers: workerSummaries, farmName: farm?.name || 'Farm' }
  })
...
export const recordPaymentFn = createServerFn({ method: 'POST' })
  ...
  .handler(async ({ data }) => {
    ...
    const { getDb } = await import('~/lib/db')
    const db = await getDb()
    const { insertWagePayment, getWorkerProfileById } = await import('./repository')
...
export const exportAttendanceCsvFn = createServerFn({ method: 'GET' })
  .inputValidator(z.object({ farmId: z.string().uuid(), startDate: z.coerce.date(), endDate: z.coerce.date() }))
  .handler(async ({ data }) => {
    const { requireAuth } = await import('~/features/auth/server-middleware')
    await requireAuth()
    const { getDb } = await import('~/lib/db')
    ...
    const records = await db
      ...
      .select(['u.name as workerName', 'wc.checkInTime', 'wc.checkOutTime', 'wc.hoursWorked', 'wc.verificationStatus'])
    ...
    return { csv, filename: `attendance-${data.startDate.toISOString().split('T')[0]}-${data.endDate.toISOString().split('T')[0]}.csv` }
  })

export const exportPayrollCsvFn = createServerFn({ method: 'GET' })
  .inputValidator(z.object({ farmId: z.string().uuid(), payrollPeriodId: z.string().uuid() }))
  .handler(async ({ data }) => {
    const { requireAuth } = await import('~/features/auth/server-middleware')
    await requireAuth()
    const { getDb } = await import('~/lib/db')
    ...
    let csv = 'Worker,Hours Worked,Wage Rate,Rate Type,Gross Wages,Paid,Balance\n'
    ...
    return { csv, filename: `payroll-${period.startDate.toISOString().split('T')[0]}-${period.endDate.toISOString().split('T')[0]}.csv` }
  })
