...
const getAssignmentsSchema = z.object({
  farmId: z.string().uuid(),
  status: z.enum(['pending', 'in_progress', 'completed', 'pending_approval', 'verified', 'rejected']).optional(),
})
...
export const assignTaskFn = createServerFn({ method: 'POST' })
  ...
  .handler(async ({ data }) => {
    ...
    const { insertTaskAssignment, getWorkerProfileById } = await import('./repository')
    ...
    await logAudit({
      ...
      details: { taskId: data.taskId, workerId: data.workerId, priority: data.priority },
    })
...
export const completeTaskFn = createServerFn({ method: 'POST' })
  ...
  .handler(async ({ data }) => {
    ...
    const { getDb } = await import('~/lib/db')
    const db = await getDb()
    const { getTaskAssignmentById, updateTaskAssignment, insertTaskPhoto, getWorkerProfileByUserId } = await import('./repository')
    const { validateTaskCompletion, determineCompletionStatus } = await import('./task-service')
    const { createDigitalForemanNotification } = await import('./notifications')
    ...
    const profile = await getWorkerProfileByUserId(db, session.user.id, assignment.farmId)
    if (!profile || profile.id !== assignment.workerId) throw new AppError('NOT_TASK_ASSIGNEE')

    const validation = validateTaskCompletion(
      { id: assignment.id, workerId: assignment.workerId, status: assignment.status, requiresPhoto: assignment.requiresPhoto, dueDate: assignment.dueDate || undefined },
      assignment.workerId,
      !!data.photoData
    )
    if (!validation.valid) throw new AppError('VALIDATION_ERROR', { metadata: { error: validation.error } })
    ...
    if (assignment.assignedBy) {
      await createDigitalForemanNotification(db, {
        ...
        type: 'taskCompleted',
        title: newStatus === 'pending_approval' ? 'Task Pending Approval' : 'Task Completed',
        message: `${session.user.name} has completed a task${newStatus === 'pending_approval' ? ' and it requires your approval' : ''}`,
        actionUrl: newStatus === 'pending_approval' ? '/task-assignments?status=pending_approval' : '/task-assignments',
      })
    }
  })
...
export const approveTaskFn = createServerFn({ method: 'POST' })
  ...
  .handler(async ({ data }) => {
    ...
    const { getDb } = await import('~/lib/db')
    const db = await getDb()
    const { getTaskAssignmentById, updateTaskAssignment, getWorkerProfileById } = await import('./repository')
    const { createDigitalForemanNotification } = await import('./notifications')
    const { logAudit } = await import('~/lib/logging/audit')
    ...
    if (assignment.status !== 'pending_approval') {
      throw new AppError('VALIDATION_ERROR', { metadata: { error: 'Task not pending approval' } })
    }
    ...
    await logAudit({
      ...
      details: { approved: data.approved, rejectionReason: data.rejectionReason },
    })

    // Send notification to worker
    const workerProfile = await getWorkerProfileById(db, assignment.workerId)
    if (workerProfile) {
      await createDigitalForemanNotification(db, {
        userId: workerProfile.userId,
        farmId: assignment.farmId,
...
export const getAssignmentsByWorkerFn = createServerFn({ method: 'GET' })
  ...
  .handler(async ({ data }) => {
    ...
    const profile = await db.selectFrom('worker_profiles').select('id').where('userId', '=', session.user.id).executeTakeFirst()
    if (!profile) return []
