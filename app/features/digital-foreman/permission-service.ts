...
export const PERMISSION_TEMPLATES = {
  ...
  health_monitor: ['mortality:log', 'vaccination:log', 'water_quality:log', 'batch:view', 'task:complete'] as const,
  full_access: ['feed:log', 'mortality:log', 'weight:log', 'vaccination:log', 'water_quality:log', 'egg:log', 'sales:view', 'task:complete', 'batch:view'] as const,
...
export function validatePermissions(permissions: Array<string>): boolean {
  ...
  return permissions.every(p => validPermissions.includes(p as ModulePermission))
}