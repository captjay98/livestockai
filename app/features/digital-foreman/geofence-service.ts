 ...
 */
export function isPointInPolygon(point: Point, vertices: Array<Point>): boolean {
  let inside = false
  const { lat, lng } = point
 ...
 */
export function calculateDistanceToPolygon(point: Point, vertices: Array<Point>): number {
 ...
 */
export function verifyLocationInGeofence(coords: Point, geofence: Geofence): LocationVerificationResult {
  ...
  if (geofence.type === 'circle') {
    ...
    return verifyPointInCircle(coords, center, geofence.radiusMeters, geofence.toleranceMeters)
  }

  // polygon type
  const inside = isPointInPolygon(coords, geofence.vertices)
  const distanceMeters = inside ? 0 : calculateDistanceToPolygon(coords, geofence.vertices)
