...
export const updateWorkerProfileFn = createServerFn({ method: 'POST' })
  ...
  .handler(async ({ data }) => {
    ...
    if (data.wageRateAmount) updates.wageRateAmount = data.wageRateAmount.toFixed(2)
...
export const checkInFn = createServerFn({ method: 'POST' })
  ...
  .handler(async ({ data }) => {
    ...
    const { getDb } = await import('~/lib/db')
    const db = await getDb()
    const { getWorkerProfileByUserId, getGeofenceByFarm, getOpenCheckIn, insertCheckIn } = await import('./repository')
    ...
    // Get worker profile
    const profile = await getWorkerProfileByUserId(db, session.user.id, data.farmId)
    ...
    if (openCheckIn && isDuplicateCheckIn(openCheckIn.checkInTime, new Date())) {
      throw new AppError('DUPLICATE_CHECK_IN')
    }

    // Verify geofence
    const geofence = await getGeofenceByFarm(db, data.farmId)
    let verificationStatus: 'verified' | 'outside_geofence' | 'manual' = 'manual'
    if (geofence) {
      const geoData = geofence.geofenceType === 'circle'
        ? { type: 'circle' as const, centerLat: Number(geofence.centerLat), centerLng: Number(geofence.centerLng), radiusMeters: Number(geofence.radiusMeters), toleranceMeters: Number(geofence.toleranceMeters) }
        : { type: 'polygon' as const, vertices: geofence.vertices || [], toleranceMeters: Number(geofence.toleranceMeters) }
      const result = verifyLocationInGeofence({ lat: data.latitude, lng: data.longitude }, geoData)
    ...
    await logAudit({
      ...
      entityId: checkInId,
      details: { verificationStatus, latitude: data.latitude, longitude: data.longitude },
    })
...
export const checkOutFn = createServerFn({ method: 'POST' })
  ...
  .handler(async ({ data }) => {
    ...
    const checkIn = await db.selectFrom('worker_check_ins').selectAll().where('id', '=', data.checkInId).executeTakeFirst()
    if (!checkIn) throw new AppError('CHECK_IN_NOT_FOUND')
    if (checkIn.checkOutTime) throw new AppError('VALIDATION_ERROR', { metadata: { error: 'Already checked out' } })
...
export const getOpenCheckInForCurrentUserFn = createServerFn({ method: 'GET' })
  ...
  .handler(async ({ data }) => {
    ...
    const { getDb } = await import('~/lib/db')
    const db = await getDb()
    const { getWorkerProfileByUserId, getOpenCheckIn } = await import('./repository')
    const { calculateHoursWorked } = await import('./attendance-service')
...
export const removeWorkerFromFarmFn = createServerFn({ method: 'POST' })
  ...
  .handler(async ({ data }) => {
    ...
    const { getDb } = await import('~/lib/db')
    const db = await getDb()
    const { getWorkerProfileById, updateWorkerProfile } = await import('./repository')
    const { logAudit } = await import('~/lib/logging/audit')
...
export const syncOfflineCheckInsFn = createServerFn({ method: 'POST' })
  ...
  .handler(async ({ data }) => {
    ...
    const { getDb } = await import('~/lib/db')
    const db = await getDb()
    const { getWorkerProfileByUserId, getGeofenceByFarm, insertCheckIn, updateCheckOut } = await import('./repository')
    const { verifyLocationInGeofence } = await import('./geofence-service')
    const { calculateHoursWorked } = await import('./attendance-service')
    const { createDigitalForemanNotification } = await import('./notifications')

    const results: Array<{ localId: string; success: boolean; serverId?: string; error?: string }> = []

    for (const checkIn of data.checkIns) {
      try {
        // Get worker profile for this farm
        const profile = await getWorkerProfileByUserId(db, session.user.id, checkIn.farmId)
        if (!profile) {
          results.push({ localId: checkIn.localId, success: false, error: 'Worker profile not found' })
          continue
        }

        // Verify geofence
        const geofence = await getGeofenceByFarm(db, checkIn.farmId)
        let verificationStatus: 'verified' | 'outside_geofence' | 'manual' | 'pending_sync' = 'manual'
        if (geofence) {
          const geoData = geofence.geofenceType === 'circle'
            ? { type: 'circle' as const, centerLat: Number(geofence.centerLat), centerLng: Number(geofence.centerLng), radiusMeters: Number(geofence.radiusMeters), toleranceMeters: Number(geofence.toleranceMeters) }
            : { type: 'polygon' as const, vertices: geofence.vertices || [], toleranceMeters: Number(geofence.toleranceMeters) }
          const result = verifyLocationInGeofence({ lat: checkIn.checkInLat, lng: checkIn.checkInLng }, geoData)
          verificationStatus = result.verified ? 'verified' : 'outside_geofence'
        }
        ...
        if (checkIn.checkOutTime && checkIn.checkOutLat && checkIn.checkOutLng) {
          const hoursWorked = calculateHoursWorked(checkIn.checkInTime, checkIn.checkOutTime)
        ...
        results.push({ localId: checkIn.localId, success: true, serverId })
      } catch (error) {
        results.push({ localId: checkIn.localId, success: false, error: String(error) })
      }
    }
