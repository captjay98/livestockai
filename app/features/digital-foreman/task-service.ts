...
export type AssignmentStatus = 'pending' | 'in_progress' | 'completed' | 'pending_approval' | 'verified' | 'rejected'
 ...
 */
export function determineCompletionStatus(requiresApproval: boolean): 'completed' | 'pending_approval' {
 ...
 */
export function isTaskOverdue(assignment: Assignment, currentTime: Date): boolean {
 ...
 */
export function calculateTaskMetrics(assignments: Array<Assignment>, currentTime: Date = new Date()): TaskMetrics {
  const total = assignments.length
  const completed = assignments.filter(a => a.status === 'completed' || a.status === 'verified').length
  const pending = assignments.filter(a => a.status === 'pending' || a.status === 'in_progress').length
  const overdue = assignments.filter(a => isTaskOverdue(a, currentTime)).length
 ...
 */
export function validatePhotoCount(currentCount: number, maxPhotos: number = 3): ValidationResult {
