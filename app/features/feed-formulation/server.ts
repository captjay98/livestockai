    ...
    .handler(async ({ data }) => {
        ...
        try {
            ...
            if (userPrices.length === 0) {
                const dbPrices = await getUserIngredientPrices(
                    db,
                    session.user.id,
                )
                ...
                applySafetyMargin(
                    {
                        ...
                        minEnergyKcalKg: requirements.minEnergyKcalKg,
                        maxFiberPercent: parseFloat(
                            requirements.maxFiberPercent,
                        ),
                        minCalciumPercent: parseFloat(
                            requirements.minCalciumPercent,
                        ),
                        ...
                        minLysinePercent: parseFloat(
                            requirements.minLysinePercent,
                        ),
                        minMethioninePercent: parseFloat(
                            requirements.minMethioninePercent,
                        ),
            ...
            const mixingInstructions =
                generateMixingInstructions(scaledIngredients)
