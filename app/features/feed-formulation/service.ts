...
export function calculateNutritionalValues(
  ...
  return ingredients.reduce(
    (totals, { ingredient, percentage }) => ({
      ...
      energy: totals.energy + (ingredient.energyKcalKg * percentage) / 100,
      fat: totals.fat + (toNumber(ingredient.fatPercent) * percentage) / 100,
      fiber:
        totals.fiber + (toNumber(ingredient.fiberPercent) * percentage) / 100,
      calcium:
        totals.calcium +
      ...
      lysine:
        totals.lysine + (toNumber(ingredient.lysinePercent) * percentage) / 100,
      methionine:
        totals.methionine +
...
export function suggestSubstitutions(
  ...
  if (infeasibilityReport.includes('fiber')) {
    suggestions.push('Reduce high-fiber ingredients or increase fiber limits')
  }
...
function getMixingOrder(ingredientName: string): number {
  ...
  if (name.includes('corn') || name.includes('wheat') || name.includes('rice'))
    return 1
  if (
