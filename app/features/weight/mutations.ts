...
export interface CreateWeightMutationInput {
  farmId: string
  data: CreateWeightSampleInput
}

export interface UpdateWeightMutationInput {
  recordId: string
  data: UpdateWeightSampleInput
}

export interface DeleteWeightMutationInput {
  recordId: string
}
...
export function useWeightMutations(): UseWeightMutationsResult {
  ...
  >({
    ...
    onSuccess: async (serverId, { data }, context) => {
      if (context.tempId) {
        ...
        await tempIdResolver.register(context.tempId, serverId, 'weight')
    ...
    onSettled: () => {
      ...
      queryClient.invalidateQueries({ queryKey: WEIGHT_QUERY_KEYS.alerts() })
    },
  })
  ...
  >({
    ...
    onMutate: async ({ recordId, data }) => {
      ...
      if (data.sampleSize !== undefined) updates.sampleSize = data.sampleSize
