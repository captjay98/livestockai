...
export async function insertReading(db: Kysely<Database>, data: ReadingInsert): Promise<string> {
...
export async function insertReadingsBatch(db: Kysely<Database>, readings: Array<ReadingInsert>): Promise<number> {
...
export async function getReadingsInRange(
  ...
  const rows = await db
    .selectFrom('sensor_readings')
    .select(['id', 'sensorId', 'value', 'recordedAt', 'isAnomaly', 'metadata'])
    .where('sensorId', '=', sensorId)
