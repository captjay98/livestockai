 ...
 */
export function getPeriodStart(date: Date, periodType: AggregationPeriod): Date {
...
export async function storeAggregation(
  ...
  const result = await db
    ...
    .onConflict(oc => 
      oc.columns(['sensorId', 'periodType', 'periodStart'])
        .doUpdateSet({
          avgValue: String(aggregation.avgValue),
          minValue: String(aggregation.minValue),
          maxValue: String(aggregation.maxValue),
...
export async function getAggregatedData(
  ...
  const rows = await db
    ...
    .select(['id', 'sensorId', 'periodType', 'periodStart', 'avgValue', 'minValue', 'maxValue', 'readingCount'])
    .where('sensorId', '=', sensorId)
