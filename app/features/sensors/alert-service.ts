...
export function isInCooldown(lastAlertTime: Date | null, cooldownMinutes: number): boolean {
...
export function calculateRateOfChange(readings: Array<SensorReading>): number {
  ...
  const timeDiffHours = (latest.recordedAt.getTime() - earliest.recordedAt.getTime()) / (1000 * 60 * 60)
