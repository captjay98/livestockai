...
export const createSensorFn = createServerFn({ method: 'POST' })
  .inputValidator(z.object({
    ...
    sensorType: z.enum(SENSOR_TYPES as unknown as [string, ...Array<string>]),
...
export const getSensorsFn = createServerFn({ method: 'GET' })
  ...
  .handler(async ({ data }) => {
    ...
    const farmIds = data.farmId ? [data.farmId] : await getUserFarms(session.user.id)
...
export const getSensorChartDataFn = createServerFn({ method: 'GET' })
  ...
  .handler(async ({ data }) => {
    ...
    return getReadingsInRange(db, data.sensorId, data.startDate, data.endDate, 500)
  })
...
export const getSensorSummaryFn = createServerFn({ method: 'GET' })
  ...
  .handler(async ({ data }) => {
    ...
    const { sensors, alertCount } = await getSensorSummaryByFarm(db, data.farmId)
    ...
    const activeSensors = sensorsWithStatus.filter(s => s.status === 'online').length
    const inactiveSensors = sensorsWithStatus.filter(s => s.status === 'offline').length
...
export const ingestReadingsFn = createServerFn({ method: 'POST' })
  ...
  .handler(async ({ data }) => {
    ...
    const { getSensorByApiKeyHash, updateLastReadingAt, incrementApiUsage } = await import('./repository')
    ...
    const now = new Date()
    await insertReadingsBatch(db, data.readings.map(r => ({
      sensorId: sensor.id,
      value: r.value,
...
export const getEnvironmentalScoreFn = createServerFn({ method: 'GET' })
  ...
  .handler(async ({ data }) => {
    ...
    if (sensors.length === 0) {
      return { score: null, factors: [], message: 'No sensors in structure' }
    }
    ...
    for (const sensor of sensors) {
      ...
      const score = Math.max(0, Math.round(100 - (deviation / maxDeviation) * 50))
    ...
    const overallScore = factorCount > 0 ? Math.round(totalScore / factorCount) : null
