...
export async function insertAlert(db: Kysely<Database>, data: AlertInsert): Promise<string> {
...
export async function getAlertsBySensor(db: Kysely<Database>, sensorId: string, limit = 50) {
  return db
...
export async function getUnacknowledgedAlerts(db: Kysely<Database>, farmId: string) {
  return db
    .selectFrom('sensor_alerts')
...
export async function acknowledgeAlert(db: Kysely<Database>, alertId: string, userId: string) {
  await db
...
export async function upsertAlertConfig(
  ...
  await db
    ...
    .values({
      sensorId,
      minThreshold: config.minThreshold != null ? String(config.minThreshold) : null,
      maxThreshold: config.maxThreshold != null ? String(config.maxThreshold) : null,
    ...
    .onConflict(oc => oc.column('sensorId').doUpdateSet({
      minThreshold: config.minThreshold != null ? String(config.minThreshold) : null,
      maxThreshold: config.maxThreshold != null ? String(config.maxThreshold) : null,
