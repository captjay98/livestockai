...
export const EGG_QUERY_KEYS = {
  ...
  poultryBatches: (farmId: string | null) =>
    ['poultry-batches-for-eggs', farmId] as const,
} as const

/**
 * Input type for creating an egg record mutation
 */
export interface CreateEggMutationInput {
  farmId: string
 ...
 */
export interface UpdateEggMutationInput {
  recordId: string
 ...
 */
export interface DeleteEggMutationInput {
  farmId: string
  recordId: string
}
...
export function useEggMutations(): UseEggMutationsResult {
  ...
  >({
    ...
    onSettled: () => {
      ...
      queryClient.invalidateQueries({ queryKey: EGG_QUERY_KEYS.summary() })
    },
  })
