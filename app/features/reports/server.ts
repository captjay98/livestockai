...
export const getProfitLossReport = createServerFn({ method: 'GET' })
    ...
    .handler(async ({ data }) => {
        ...
        try {
            const [salesByTypeResult, expensesByCategoryResult] =
                await Promise.all([
    ...
    .handler(async ({ data }) => {
        ...
        try {
            ...
            const totalPoultry = batchesWithRates
                .filter(
                    (b) =>
                        b.livestockType === 'poultry' && b.status === 'active',
                )
                .reduce((sum, b) => sum + b.currentQuantity, 0)

            const totalFish = batchesWithRates
                .filter(
                    (b) => b.livestockType === 'fish' && b.status === 'active',
                )
                .reduce((sum, b) => sum + b.currentQuantity, 0)
        ...
        try {
            ...
            return {
                ...
                summary: {
                    ...
                    byType: Array.from(byTypeMap.entries()).map(
                        ([type, typeData]) => ({
                            type,
