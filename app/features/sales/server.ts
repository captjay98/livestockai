    ...
    try {
        ...
        if (input.batchId && input.livestockType !== 'eggs') {
            ...
            return await db.transaction().execute(async (tx) => {
                ...
                await atomicDecrementBatchQuantity(
                    tx,
                    input.batchId!,
                    input.quantity,
                )
    ...
    try {
        ...
        await db.transaction().execute(async (tx) => {
            ...
            if (
                ...
                await adjustBatchQuantityOnUpdate(
                    tx,
                    sale.batchId,
                    quantityDiff,
                )
            ...
            updateData.totalAmount = calculateNewTotalAmount(
                newQuantity,
                newPrice,
            )
...
export async function getSalesSummary(
    ...
    const { checkFarmAccess, getUserFarms } =
        await import('~/features/auth/utils')
    const { AppError } = await import('~/lib/errors')
    const { getSalesSummary: getSalesSummaryRepo } =
        await import('./repository')
...
export async function getTotalRevenue(
    ...
    const { verifyFarmAccess } = await import('~/features/auth/utils')
    const { AppError } = await import('~/lib/errors')
    const { getTotalRevenue: getTotalRevenueRepo } =
        await import('./repository')
    ...
    .handler(async ({ data }) => {
        ...
        const [paginatedSales, summary, batches, customers] = await Promise.all(
            [
                ...
                (async () => {
                    const { getCustomers } =
                        await import('~/features/customers/server')
