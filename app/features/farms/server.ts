      ...
      farmBatches.reduce((acc, batch) => {
        const rate =
          batch.initialQuantity > 0
            ? (Number(batch.totalMortality) / batch.initialQuantity) * 100
            : 0
        return acc + rate
  ...
  .handler(async ({ data }) => {
    ...
    return {
      ...
      sensorSummary: {
        ...
        activeSensors: sensorsWithStatus.filter((s) => s.status === 'online')
          .length,
        inactiveSensors: sensorsWithStatus.filter((s) => s.status === 'offline')
          .length,
