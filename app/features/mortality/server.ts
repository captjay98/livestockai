    ...
    try {
        ...
        if (data.quantity > batch.currentQuantity) {
            throw new AppError('INSUFFICIENT_STOCK', {
                message:
                    'Mortality quantity cannot exceed current batch quantity',
                metadata: {
    ...
    try {
        ...
        const trends = await db
            ...
            .select([
                (eb) =>
                    eb
                        .fn('to_char', [eb.ref('date'), eb.val(dateFormat)])
                        .as('period'),
            ...
            .groupBy((eb) =>
                eb.fn('to_char', [eb.ref('date'), eb.val(dateFormat)]),
            )
            .orderBy(
    ...
    try {
        ...
        if (query.search) {
            ...
            baseQuery = baseQuery.where((eb) =>
                eb.or([
                    eb(
                        sql.raw('mortality_records.cause'),
                        'ilike',
                        searchLower,
                    ),
        ...
        if (query.sortBy) {
            ...
            const sortColumn =
                allowedCols[query.sortBy] || 'mortality_records.date'
...
export const getMortalityDataForFarmFn = createServerFn({ method: 'GET' })
    ...
    .handler(async ({ data }) => {
        ...
        const [paginatedRecords, alerts, summary, allBatches] =
            await Promise.all([
