...
export const getTasksFn = createServerFn({ method: 'GET' })
    .inputValidator((data: { farmId: string; frequency?: 'daily' | 'weekly' | 'monthly' }) => data)
    .handler(async ({ data }) => {
 ...
 */
export async function completeTask(userId: string, taskId: string): Promise<string> {
    const { db } = await import('~/lib/db')
    const { checkFarmAccess } = await import('../auth/utils')
    ...
    try {
        ...
        const periodStart = getPeriodStart(new Date(), task.frequency as 'daily' | 'weekly' | 'monthly')
