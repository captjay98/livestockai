 ...
 */
export function validateListingInput(input: CreateListingInput): { valid: boolean; errors: string[] } {
  const errors: string[] = []

  if (!input.livestockType) errors.push('Livestock type is required')
  if (!input.species?.trim()) errors.push('Species is required')
  if (!input.quantity || input.quantity <= 0) errors.push('Quantity must be greater than 0')
  if (input.minPrice < 0) errors.push('Minimum price cannot be negative')
  if (input.maxPrice < input.minPrice) errors.push('Maximum price must be greater than or equal to minimum price')
 ...
 */
export function calculateExpirationDate(createdAt: Date, expirationDays: 7 | 14 | 30 | 60): Date {
 ...
 */
export function isListingExpired(expiresAt: Date, now: Date = new Date()): boolean {
 ...
 */
export function shouldNotifyExpiration(expiresAt: Date, now: Date = new Date()): boolean {
 ...
 */
export function validateStatusTransition(currentStatus: ListingStatus, newStatus: ListingStatus): boolean {
