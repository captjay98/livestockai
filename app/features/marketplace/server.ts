    ...
    .handler(async ({ data }) => {
        ...
        try {
            ...
            const listingId = await insertListing(db, {
                ...
                photoUrls: null,
                fuzzingLevel: 'medium',
                contactPreference: data.contactMethod === 'both' ? 'both' : data.contactMethod === 'phone' ? 'phone' : 'app',
                batchId: null,
                status: 'active',
                expiresAt,
            })
    ...
    .handler(async ({ data }) => {
        try {
            ...
            const fuzzedListings = result.data.map((listing) =>
                fuzzListing(listing as any, null),
            )
...
export const getListingDetailFn = createServerFn({ method: 'GET' })
    ...
    .handler(async ({ data }) => {
        try {
            ...
            let result: any = listing
            if (!isOwner) {
                const { fuzzListing } = await import('./privacy-fuzzer')
                result = fuzzListing(listing as any, null) as any
            }
...
export const deleteListingFn = createServerFn({ method: 'POST' })
    ...
    .handler(async ({ data }) => {
        ...
        try {
            ...
            const { notifyContactRequestersOfDeletion } =
...
export const respondToRequestFn = createServerFn({ method: 'POST' })
    ...
    .handler(async ({ data }) => {
        ...
        try {
            ...
            const request = { sellerId: session.user.id, status: 'pending', species: 'livestock', buyerId: '', listingId: '' } as any
            ...
            const updatedRequest = { ...request, status: data.approved ? 'approved' : 'denied' }
...
export const getContactRequestsFn = createServerFn({ method: 'GET' })
    ...
    .handler(async ({ data }) => {
        ...
        try {
            ...
            const result = { data: [], total: 0, page: data.page, pageSize: data.pageSize }
...
export const checkExpiringListingsFn = createServerFn({ method: 'POST' })
    ...
    .handler(async ({ data }) => {
        try {
            ...
            const expiringListings = await getExpiringListings(
                db,
                data.daysFromNow,
            )
