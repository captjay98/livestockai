...
export async function getListingAnalytics(
  db: Kysely<Database>,
  listingId: string,
): Promise<{ viewCount: number; contactCount: number; conversionRate: number }> {
  const result = await db
    .selectFrom('marketplace_listings')
  ...
  const conversionRate = result.viewCount > 0 ? result.contactCount / result.viewCount : 0
...
export async function getExpiringListings(
  db: Kysely<Database>,
  daysFromNow: number,
): Promise<Array<{ id: string; sellerId: string; species: string; expiresAt: Date }>> {
