...
export function EggReportView({ report }: { report: EggReport }) {
  ...
  const columns = useMemo<Array<ColumnDef<any>>>(
    () => [
      {
        accessorKey: 'date',
        header: t('reports:eggs.columns.date', { defaultValue: 'Date' }),
        cell: ({ row }) => formatDate(row.original.date),
      },
      {
        accessorKey: 'species',
        header: t('reports:eggs.columns.species', { defaultValue: 'Flock' }),
        cell: ({ row }) => (
      ...
      {
        accessorKey: 'cracked',
        header: t('reports:eggs.columns.cracked', { defaultValue: 'Cracked' }),
        cell: ({ row }) => row.original.cracked.toLocaleString(),
      },
      {
        accessorKey: 'usable',
        header: t('reports:eggs.columns.usable', { defaultValue: 'Usable' }),
