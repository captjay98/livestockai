...
export function SalesReportView({ report }: { report: SalesReport }) {
  ...
  const columns = useMemo<Array<ColumnDef<any>>>(
    () => [
      {
        accessorKey: 'date',
        header: t('reports:sales.columns.date', { defaultValue: 'Date' }),
        cell: ({ row }) => formatDate(row.original.date),
      },
      {
        accessorKey: 'livestockType',
        header: t('reports:sales.columns.type', { defaultValue: 'Type' }),
        cell: ({ row }) => (
      ...
      },
      {
        ...
        header: t('reports:sales.columns.price', { defaultValue: 'Price' }),
        cell: ({ row }) => formatCurrency(row.original.unitPrice),
      },
      {
        accessorKey: 'totalAmount',
        header: t('reports:sales.columns.total', { defaultValue: 'Total' }),
