    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-md">
              ...
              <SelectContent>
                {workers.map((worker) => (
                  <SelectItem key={worker.id} value={worker.id}>
                    {worker.userName || 'Unknown'} ({format(worker.balance)} due)
                  </SelectItem>
                ))}
              </SelectContent>
            </Select>
          ...
          {selectedWorker && selectedWorker.balance > 0 && (
            <div className="bg-muted p-3 rounded-lg flex items-center justify-between">
              <div>
                ...
                <div className="text-lg font-bold">{format(selectedWorker.balance)}</div>
              </div>
              <Button variant="outline" size="sm" onClick={handlePayFullBalance}>
                Pay Full
              </Button>
            </div>
          )}
        ...
        <DialogFooter>
          ...
          <Button onClick={handleSubmit} disabled={recordPayment.isPending} className="min-h-[48px]">
            Record Payment
          </Button>
        </DialogFooter>
      </DialogContent>
