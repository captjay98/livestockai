...
export function CheckInButton({ farmId, checkInId, isCheckedIn }: CheckInButtonProps) {
  ...
  const handleClick = async () => {
    ...
    try {
      ...
      if (isCheckedIn && checkInId) {
        await checkOut.mutateAsync({ data: { checkInId, latitude, longitude } })
      } else {
        await checkIn.mutateAsync({ data: { farmId, latitude, longitude, accuracy } })
      }
    } catch (error) {
