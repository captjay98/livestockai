        ...
        <div className="space-y-4">
            <div>
                ...
                <Select
                    value={template}
                    onValueChange={(v) =>
                        handleTemplateChange(v as TemplateName)
                    }
                >
                ...
                <p className="text-xs text-muted-foreground mt-1">
                    {template === 'feed_handler' &&
                        'Can log feed and complete tasks'}
                    {template === 'health_monitor' &&
                        'Can log mortality, vaccinations, and water quality'}
                    {template === 'full_access' &&
                        'Access to all worker functions'}
                    {template === 'custom' &&
            ...
            <div className="text-sm text-muted-foreground">
                {value.length} permission{value.length !== 1 ? 's' : ''}{' '}
                selected
            </div>
        </div>
    )
