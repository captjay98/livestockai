...
import { PERMISSION_TEMPLATES, getPermissionsFromTemplate } from '~/features/digital-foreman/permission-service'
import { Label } from '~/components/ui/label'
import { Select, SelectContent, SelectItem, SelectTrigger, SelectValue } from '~/components/ui/select'
import { Checkbox } from '~/components/ui/checkbox'

const ALL_PERMISSIONS: Array<{ key: ModulePermission; label: string; description: string }> = [
  { key: 'feed:log', label: 'Log Feed', description: 'Record feed consumption' },
  { key: 'mortality:log', label: 'Log Mortality', description: 'Record deaths' },
  { key: 'weight:log', label: 'Log Weight', description: 'Record weight samples' },
  { key: 'vaccination:log', label: 'Log Vaccination', description: 'Record vaccinations' },
  { key: 'water_quality:log', label: 'Log Water Quality', description: 'Record water tests' },
  { key: 'egg:log', label: 'Log Eggs', description: 'Record egg production' },
  { key: 'sales:view', label: 'View Sales', description: 'View sales records' },
  { key: 'task:complete', label: 'Complete Tasks', description: 'Mark tasks as done' },
  { key: 'batch:view', label: 'View Batches', description: 'View batch details' },
]
...
export function PermissionTemplateSelector({ value, onChange }: PermissionTemplateSelectorProps) {
  const [template, setTemplate] = useState<TemplateName>('custom')

  // Detect if current permissions match a template
  useEffect(() => {
    const templateNames = Object.keys(PERMISSION_TEMPLATES) as Array<keyof typeof PERMISSION_TEMPLATES>
  ...
  const handlePermissionToggle = (permission: ModulePermission, checked: boolean) => {
    if (checked) {
  ...
  return (
    <div className="space-y-4">
      <div>
        ...
        <Select value={template} onValueChange={(v) => handleTemplateChange(v as TemplateName)}>
        ...
        <p className="text-xs text-muted-foreground mt-1">
          ...
          {template === 'health_monitor' && 'Can log mortality, vaccinations, and water quality'}
