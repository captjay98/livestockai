...
import { getGeofenceFn, saveGeofenceFn } from '~/features/digital-foreman/server-payroll'
  ...
  return (
    <Card>
      ...
      <CardContent className="space-y-4">
        <div>
          <Label>Type</Label>
          <Select value={type} onValueChange={(v) => v && setType(v as GeofenceType)}>
          ...
          <>
            <div className="grid grid-cols-2 gap-4">
              <div>
                ...
                <Input type="number" step="0.0000001" value={centerLat} onChange={(e) => setCenterLat(Number(e.target.value))} />
              </div>
              <div>
                <Label>Center Longitude</Label>
                <Input type="number" step="0.0000001" value={centerLng} onChange={(e) => setCenterLng(Number(e.target.value))} />
              </div>
            </div>
            <div>
              <Label>Radius (meters): {radius}m</Label>
              <input type="range" min={50} max={5000} value={radius} onChange={(e) => setRadius(Number(e.target.value))} className="w-full" />
            </div>
          </>
        )}
        ...
        <Button onClick={handleSave} disabled={save.isPending}>Save Geofence</Button>
      </CardContent>
    </Card>
  )
}
