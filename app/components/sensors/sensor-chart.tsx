...
import { getMortalityForChartFn, getSensorChartDataFn } from '~/features/sensors/server'
...
export function SensorChart({ sensorId, sensorType, structureId, showMortality = false }: SensorChartProps) {
  const [timeRange, setTimeRange] = useState<TimeRange>('24h')
  const [data, setData] = useState<Array<{ timestamp: string; value: number; mortality?: number }>>([])
    ...
    const fetchData = async () => {
      ...
      try {
        ...
        const [sensorData, mortalityData] = await Promise.all([
          getSensorChartDataFn({ data: { sensorId, startDate, endDate } }),
          showMortality && structureId
            ? getMortalityForChartFn({ data: { structureId, startDate, endDate } })
            : Promise.resolve([]),
        ])
        ...
        for (const m of mortalityData) {
          ...
          mortalityByDate.set(dateKey, (mortalityByDate.get(dateKey) ?? 0) + m.quantity)
        }
