...
export function SensorCard({ sensor, latestReading, readings }: SensorCardProps) {
  ...
  const chartData = readings?.map(r => ({ value: r.value, time: r.recordedAt.getTime() })) || []
  const minValue = chartData.length ? Math.min(...chartData.map(d => d.value)) : 0
    ...
    <Card>
      <CardContent className="p-4">
        <div className="flex items-center justify-between mb-3">
          ...
          <div className={`w-2 h-2 rounded-full ${STATUS_COLORS[sensor.status]}`} />
        </div>
            ...
            <ResponsiveContainer width="100%" height={32}>
              <LineChart data={chartData}>
                <Line 
                  ...
                  dot={(props) => {
                    const { payload } = props
                    if (payload.value === minValue || payload.value === maxValue) {
                      return <circle cx={props.cx} cy={props.cy} r={2} fill={STROKE_COLORS[sensor.status]} />
                    }
                    return null
                  }}
                />
