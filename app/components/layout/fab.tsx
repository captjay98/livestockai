      ...
      <DropdownMenu>
        <DropdownMenuTrigger asChild>
          ...
          >
            ...
            <span className="sr-only">{t('common:actions', { defaultValue: 'Quick Actions' })}</span>
          </Button>
        </DropdownMenuTrigger>
        <DropdownMenuContent align="end" className="w-56" sideOffset={10}>
          <DropdownMenuLabel>
            {batchId 
              ? t('common:actions', { defaultValue: 'Batch Actions' }) 
              : t('common:quickActions', { defaultValue: 'Quick Actions' })
            }
          </DropdownMenuLabel>
          <DropdownMenuSeparator />
          
          <DropdownMenuItem onClick={() => handleAction('feed')} className="gap-2 p-3 cursor-pointer">
            <Wheat className="h-4 w-4" />
            <span>{t('common:logFeed', { defaultValue: 'Log Feed' })}</span>
          </DropdownMenuItem>
          
          <DropdownMenuItem onClick={() => handleAction('water')} className="gap-2 p-3 cursor-pointer">
            <Droplets className="h-4 w-4" />
            <span>{t('common:logWater', { defaultValue: 'Log Water' })}</span>
          </DropdownMenuItem>
          
          <DropdownMenuItem onClick={() => handleAction('mortality')} className="gap-2 p-3 cursor-pointer">
            <HeartPulse className="h-4 w-4 text-red-500" />
            <span>{t('common:logMortality', { defaultValue: 'Log Mortality' })}</span>
          </DropdownMenuItem>
          
          <DropdownMenuSeparator />
          
          <DropdownMenuItem onClick={() => handleAction('sale')} className="gap-2 p-3 cursor-pointer">
           ...
           {!batchId && (
             <>
               <DropdownMenuSeparator />
               <DropdownMenuItem onClick={() => navigate({ to: '/batches/new' })} className="gap-2 p-3 cursor-pointer">
                 <ClipboardList className="h-4 w-4" />
                 <span>{t('batches:create', { defaultValue: 'New Batch' })}</span>
               </DropdownMenuItem>
             </>
           )}
        </DropdownMenuContent>
