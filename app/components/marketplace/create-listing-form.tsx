...
export function CreateListingForm({ onSubmit, batches, isSubmitting }: CreateListingFormProps) {
  ...
  const canGoNext = () => {
    switch (currentStep) {
      case 1: return formData.livestockType && formData.species
      case 2: return formData.quantity && formData.minPrice
      case 3: return formData.address || formData.coordinates
    ...
    if (type === 'price' && formData.minPrice) {
      return (
        <div className="mt-2 p-2 bg-muted rounded text-sm">
          ...
          {format(Math.floor(Number(formData.minPrice) * 0.9))} - {format(Math.ceil(Number(formData.maxPrice || formData.minPrice) * 1.1))}
        </div>
      )
    }
    if (type === 'location' && formData.address) {
      const fuzzedAddress = formData.address.split(' ').slice(0, -1).join(' ') + ' Area'
      return (
        <div className="mt-2 p-2 bg-muted rounded text-sm">
          <span className="text-muted-foreground">Preview: </span>
          {fuzzedAddress}
        ...
        {currentStep === 1 && (
          <div className="space-y-4">
            ...
            <div>
              ...
              <Select value={formData.livestockType} onValueChange={(value) => updateFormData('livestockType', value)}>
                <SelectTrigger>
        ...
        {currentStep === 3 && (
          <div className="space-y-4">
            ...
            <div>
              ...
              <Select value={formData.fuzzingLevel} onValueChange={(value) => updateFormData('fuzzingLevel', value)}>
                <SelectTrigger>
                  <SelectValue />
                </SelectTrigger>
                <SelectContent>
                  <SelectItem value="low">Low - Show approximate area</SelectItem>
                  <SelectItem value="medium">Medium - Show general vicinity</SelectItem>
                  <SelectItem value="high">High - Show only city/region</SelectItem>
                </SelectContent>
              </Select>
            </div>
        ...
        {currentStep === 4 && (
          <div className="space-y-4">
            ...
            <div className="grid grid-cols-2 gap-4">
              ...
              <div>
                <Label htmlFor="expirationPeriod">Listing Duration (days)</Label>
                <Select value={formData.expirationPeriod} onValueChange={(value) => updateFormData('expirationPeriod', value)}>
                  <SelectTrigger>
                    <SelectValue />
                  </SelectTrigger>
                  <SelectContent>
