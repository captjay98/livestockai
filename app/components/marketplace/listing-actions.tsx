...
export function ListingActions({ listing, onAction }: ListingActionsProps) {
  ...
  const getAvailableActions = () => {
    ...
    if (listing.status === 'active') {
      actions.push({ key: 'pause', label: t('actions.pause'), icon: Pause })
      actions.push({ key: 'mark-sold', label: t('actions.markAsSold'), icon: CheckCircle })
    }
    
    if (listing.status === 'paused') {
      actions.push({ key: 'resume', label: t('actions.resume'), icon: Play })
    }
    
    if (listing.status === 'active' || listing.status === 'paused') {
      actions.push({ key: 'extend', label: t('actions.extend'), icon: Clock })
    }
    
    actions.push({ key: 'delete', label: t('actions.delete'), icon: Trash2 })
