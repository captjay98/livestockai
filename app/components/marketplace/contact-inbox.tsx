        ...
        {requests.length === 0 ? (
          ...
          requests.map((request) => (
            <RequestCard
              key={request.id}
              request={request}
              onApprove={request.status === 'pending' ? (message) => onRespond(request.id, true, message) : undefined}
              onDeny={request.status === 'pending' ? (message) => onRespond(request.id, false, message) : undefined}
            />
          ))
        )}
      </TabsContent>
