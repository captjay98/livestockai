...
export function useRegisterSW() {
  ...
  useEffect(() => {
    ...
    const registerSW = async () => {
        ...
        if (pwaModule?.registerSW) {
          pwaModule.registerSW({
            ...
            onRegistered(r: ServiceWorkerRegistration | undefined) {
              setState((prev) => ({ ...prev, registration: r || null }))
              if (r) setInterval(() => r.update().catch(() => {}), 3600000)
            },
      ...
      try {
        ...
        setState((prev) => ({
          ...prev,
          error: e instanceof Error ? e : new Error('Registration failed'),
        }))
      }
    }
  ...
  return { ...state, updateServiceWorker, dismissRefresh, dismissOfflineReady }
}
