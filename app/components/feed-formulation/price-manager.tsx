...
export function PriceManager() {
  ...
  const { data: priceHistory } = useQuery({
    ...
    queryFn: async () => {
      ...
      const { getPriceHistoryFn } =
        await import('~/features/feed-formulation/server')
      return getPriceHistoryFn({ data: { ingredientId: selectedIngredient } })
    },
  ...
  const updatePriceMutation = useMutation({
    mutationFn: async (data: { ingredientId: string; pricePerKg: number }) => {
  ...
  const importCsvMutation = useMutation({
    ...
    onError: (error) => {
      toast.error(
        t('priceManager.importError', { defaultValue: 'Import Failed' }),
        {
          description:
      ...
      <Card>
        <CardHeader>
          <CardTitle className="flex items-center gap-2">
            ...
            {t('priceManager.title', { defaultValue: 'Price Manager' })}
          </CardTitle>
        </CardHeader>
        <CardContent>
          ...
          <div className="flex flex-col sm:flex-row gap-2 sm:items-center">
            ...
            <span className="text-xs text-muted-foreground">
              {t('priceManager.csvFormat', {
                defaultValue: 'Format: ingredient_name,price_per_kg',
              })}
            </span>
          </div>
        </CardHeader>
                ...
                >
                  <div className="flex items-center gap-3">
                    ...
                    <Badge variant="outline" className="ml-2">
                      Max {ingredient.maxInclusionPercent || 100}%
                    </Badge>
                  </div>
                          ...
                          <Button
                            ...
                            onClick={(e) => {
                              ...
                              handleEditStart(
                                ingredient.id,
                                ingredient.pricePerKg
                                  ? toNumber(ingredient.pricePerKg)
                                  : null,
                              )
                            }}
                          >
