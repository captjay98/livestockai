    ...
    <Card className="p-6 space-y-6">
      ...
      <div className="space-y-4">
        <div className="space-y-2">
          <Label htmlFor="paymentTerms">{t('business.paymentTerms')}</Label>
          <Input
            id="paymentTerms"
            type="number"
            min="0"
            ...
            onChange={(e) => {
              ...
              if (val === '') {
                onSettingsChange({ defaultPaymentTermsDays: NaN })
              } else {
                onSettingsChange({ defaultPaymentTermsDays: parseInt(val) })
              }
            }}
            onBlur={(e) => {
              const val = parseInt(e.target.value)
              if (isNaN(val)) {
                onSettingsChange({ defaultPaymentTermsDays: 30 })
              }
            }}
          />
        ...
        <div className="space-y-2">
          ...
          <Select
            ...
            onValueChange={(value) =>
              value &&
              onSettingsChange({ fiscalYearStartMonth: parseInt(value) })
            }
          >
