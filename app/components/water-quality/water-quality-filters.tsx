    ...
    <div className="flex items-center gap-2">
      <DropdownMenu>
        ...
        <DropdownMenuContent align="end" className="w-48">
          <DropdownMenuLabel>
            {t('waterQuality:filterBy', { defaultValue: 'Filter by' })}
          </DropdownMenuLabel>
          <DropdownMenuSeparator />
          <DropdownMenuRadioGroup
            value={dateRange}
            onValueChange={(value) =>
              onDateRangeChange?.(value as 'all' | 'week' | 'month')
            }
          >
            <DropdownMenuRadioItem value="all">
              {t('waterQuality:allRecords', { defaultValue: 'All Records' })}
            </DropdownMenuRadioItem>
            <DropdownMenuRadioItem value="week">
              {t('waterQuality:lastWeek', { defaultValue: 'Last 7 Days' })}
            </DropdownMenuRadioItem>
            <DropdownMenuRadioItem value="month">
              {t('waterQuality:lastMonth', { defaultValue: 'Last 30 Days' })}
            </DropdownMenuRadioItem>
          </DropdownMenuRadioGroup>
        </DropdownMenuContent>
      </DropdownMenu>
