 ...
 */
export function getAvailableLivestockTypes(enabledModules: Array<ModuleKey>): Array<LivestockType> {
  return enabledModules.flatMap((moduleKey) => MODULE_METADATA[moduleKey].livestockTypes)
}

/**
 * Helper function to get species options for a livestock type
 */
export function getSpeciesForLivestockType(livestockType: LivestockType): Array<string> {
...
describe('Batch Form - Property Tests', () => {
  ...
  const moduleSetArb = fc.uniqueArray(moduleKeyArb, { minLength: 1, maxLength: 6 })
      ...
      fc.property(moduleSetArb, (enabledModules) => {
        ...
        availableTypes.forEach((type) => {
          const hasModule = enabledModules.some((moduleKey) =>
            MODULE_METADATA[moduleKey].livestockTypes.includes(type),
          )
        ...
        expect(availableTypes.length).toBeGreaterThanOrEqual(enabledModules.length)
      }),
      { numRuns: 100 },
    )
  })
    ...
    fc.assert(
      fc.property(moduleSetArb, (enabledModules) => {
        ...
        enabledModules.forEach((moduleKey) => {
          const moduleTypes = MODULE_METADATA[moduleKey].livestockTypes
          // At least one of the module's types should be in available types
          const hasType = moduleTypes.some((type) => availableTypes.includes(type))
