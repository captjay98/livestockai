                  ...
                  <Input
                    ...
                    placeholder={
                      ingredient.userPrice
                        ? format(ingredient.userPrice.pricePerKg)
                        : 'Enter price'
                    }
                    value={prices[ingredient.id] || ''}
                    onChange={(e) =>
                      setPrices({ ...prices, [ingredient.id]: e.target.value })
                    }
                    className="h-12 w-32 text-lg"
                  />
                  <Button
                    onClick={() => handleSave(ingredient.id)}
                    disabled={!prices[ingredient.id] || saving[ingredient.id]}
                    className="h-12 min-w-[80px]"
                  >
                    <Save className="h-4 w-4 mr-2" />
                    {saving[ingredient.id] ? 'Saving...' : 'Save'}
                  </Button>
                </div>
              </div>
            ))}
