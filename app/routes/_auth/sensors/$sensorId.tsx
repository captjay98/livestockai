...
export const Route = createFileRoute('/_auth/sensors/$sensorId' as any)({
  loader: async ({ params }: { params: { sensorId: string } }) => {
    const sensor = await getSensorFn({ data: { sensorId: params.sensorId } })
    const now = new Date()
    const dayAgo = new Date(now.getTime() - 24 * 60 * 60 * 1000)
    const readings = await getSensorChartDataFn({
      data: { sensorId: params.sensorId, startDate: dayAgo, endDate: now }
    })
...
function SensorDetailPage() {
  ...
  const handleUpdate = async (formData: {
    ...
    navigate({ to: '/sensors/$sensorId' as any, params: { sensorId } as any })
    return { sensorId }
  }
  ...
  return (
    <div className="container py-6 space-y-6">
      <div className="flex items-center gap-4">
        <Button variant="ghost" size="sm" onClick={() => navigate({ to: '/sensors' as any })}>
        ...
        <AlertDialog>
          ...
          <AlertDialogContent>
            <AlertDialogHeader>
              ...
              <AlertDialogDescription>
                This will delete the sensor. Historical data will be retained for 90 days.
              </AlertDialogDescription>
            </AlertDialogHeader>
            <AlertDialogFooter>
