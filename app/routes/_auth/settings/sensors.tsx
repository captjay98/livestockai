      ...
      <div className="grid gap-6 lg:grid-cols-2">
        <Card>
          ...
          <CardContent className="space-y-4">
            {(Object.entries(SENSOR_TYPE_CONFIG) as Array<[SensorType, typeof SENSOR_TYPE_CONFIG[SensorType]]>).map(
              ([type, config]) => (
                <div key={type} className="flex items-center justify-between py-2 border-b last:border-0">
                  <div>
                    <p className="font-medium">{config.label}</p>
                    <p className="text-sm text-muted-foreground">
                      {config.defaultThresholds.min} - {config.defaultThresholds.max} {config.unit}
                    </p>
                  </div>
                </div>
              )
