...
import { getCustomers, getTopCustomers, createCustomer } from '~/lib/customers/server'
import { requireAuth } from '~/lib/auth/middleware'
import { formatNaira } from '~/lib/currency'
import { Button } from '~/components/ui/button'
import { Card, CardContent, CardDescription, CardHeader, CardTitle } from '~/components/ui/card'
import { Badge } from '~/components/ui/badge'
import { Plus, Users, Phone, MapPin, TrendingUp, Mail } from 'lucide-react'
...
function CustomersPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const result = await createCustomerAction({ data: formData })
      if (result.success) {
        setDialogOpen(false)
        resetForm()
        router.invalidate()
          ...
          <CardContent>
            <div className="space-y-3">
              {topCustomers.map((customer, index) => (
                <div key={customer.id} className="flex items-center justify-between p-3 border rounded-lg glass">
          ...
          <CardContent>
            <div className="space-y-3 sm:space-y-4">
              {customers.map((customer) => (
                <div key={customer.id} className="flex flex-col sm:flex-row sm:items-center justify-between p-3 sm:p-4 glass rounded-lg hover:bg-muted/30 transition-colors gap-3">
                  ...
                  <div className="flex gap-2 sm:ml-4">
                    <Button variant="outline" size="sm" className="flex-1 sm:flex-none min-h-[44px]" onClick={() => handleViewCustomer(customer)}>View</Button>
                    <Button variant="outline" size="sm" className="flex-1 sm:flex-none min-h-[44px]" onClick={() => handleEditCustomer(customer)}>Edit</Button>
                  </div>
                </div>
              ))}
            </div>
