...
export const ingestReadingsFn = createServerFn({ method: 'POST' })
    ...
    .handler(async ({ data }) => {
        ...
        const { updateLastReadingAt } =
            await import('~/features/sensors/repository')
        const { insertReadingsBatch } =
            await import('~/features/sensors/readings-repository')
        const { verifyApiKey, validateReadingValue } =
        ...
        const toInsert = readings.map((r) => {
            const { isAnomaly } = validateReadingValue(
                r.value,
                sensor.sensorType,
            )
            return {
        ...
        const { processReadingForAlerts } =
            await import('~/features/sensors/alert-processor')
