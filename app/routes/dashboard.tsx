...
import { requireAuth, getUserFarms } from '~/lib/auth/middleware'
  ...
  if (hasFarms === false) {
    return (
      <div className="flex flex-col items-center justify-center min-h-[60vh] text-center p-4 sm:p-8">
        ...
        <h1 className="text-2xl sm:text-3xl font-bold tracking-tight mb-3">Welcome to JayFarms</h1>
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col gap-3">
        <div>
          ...
          <p className="text-muted-foreground text-sm sm:text-base">Here's what's happening on your farms.</p>
        </div>
      ...
      {!stats ? (
        <Card className="border-dashed border-2 bg-muted/30">
          <CardContent className="py-12 sm:py-16 text-center">
            ...
            <p className="text-muted-foreground mb-4">Start adding batches to see your metrics.</p>
              ...
              <CardContent className="p-4">
                <div className="flex items-start justify-between">
                  <div className="space-y-1">
                    ...
                    <p className="text-xl sm:text-2xl font-bold">{formatNaira(stats.financial.monthlyRevenue)}</p>
                    {stats.financial.revenueChange !== undefined && (
                      <div className="flex items-center gap-1 text-xs">
                        <span className={cn(
                          "flex items-center gap-0.5 font-medium",
                          stats.financial.revenueChange >= 0 ? "text-emerald-600" : "text-red-600"
                        )}>
                          ...
                          {stats.financial.revenueChange >= 0 ? '+' : ''}{stats.financial.revenueChange.toFixed(1)}%
                        </span>
                      </div>
                    )}
                  </div>
              ...
              <CardContent className="p-4">
                <div className="flex items-start justify-between">
                  <div className="space-y-1">
                    ...
                    <p className="text-xl sm:text-2xl font-bold">{formatNaira(stats.financial.monthlyExpenses)}</p>
                    {stats.financial.expensesChange !== undefined && (
                      <div className="flex items-center gap-1 text-xs">
                        <span className={cn(
                          "flex items-center gap-0.5 font-medium",
                          stats.financial.expensesChange >= 0 ? "text-red-600" : "text-emerald-600"
                        )}>
                          ...
                          {stats.financial.expensesChange >= 0 ? '+' : ''}{stats.financial.expensesChange.toFixed(1)}%
                        </span>
                      </div>
                    )}
                  </div>
              ...
              <CardContent className="p-4">
                <div className="flex items-start justify-between">
                  <div className="space-y-1">
                    ...
                    <p className="text-xl sm:text-2xl font-bold">{formatNaira(stats.financial.monthlyProfit)}</p>
              ...
              <CardContent className="p-4">
                <div className="flex items-start justify-between">
                  <div className="space-y-1">
                    ...
                    <p className="text-xl sm:text-2xl font-bold">{stats.inventory.activeBatches}</p>
              ...
              <CardContent className="p-4">
                <div className="flex items-center gap-3">
                  ...
                  <div className="min-w-0">
                    ...
                    <p className="text-lg sm:text-xl font-bold">{stats.inventory.totalPoultry.toLocaleString()}</p>
                  </div>
                </div>
              </CardContent>
            </Card>
              ...
              <CardContent className="p-4">
                <div className="flex items-center gap-3">
                  ...
                  <div className="min-w-0">
                    ...
                    <p className="text-lg sm:text-xl font-bold">{stats.inventory.totalFish.toLocaleString()}</p>
                  </div>
                </div>
              </CardContent>
            </Card>
              ...
              <CardContent className="p-4">
                <div className="flex items-center gap-3">
                  ...
                  <div className="min-w-0">
                    ...
                    <p className="text-lg sm:text-xl font-bold">{stats.production.eggsThisMonth.toLocaleString()}</p>
                  </div>
                </div>
              </CardContent>
            </Card>
                ...
                <Card className="border-amber-200 bg-amber-50 dark:border-amber-900/50 dark:bg-amber-950/20">
                  <CardHeader className="pb-2">
                    <CardTitle className="flex items-center gap-2 text-sm text-amber-700 dark:text-amber-400">
                      ...
                      Alerts
                      <Badge variant="outline" className="ml-auto bg-amber-100 dark:bg-amber-900/50 text-amber-700 dark:text-amber-400 border-amber-300 dark:border-amber-700">
                        {(stats.alerts.highMortality.length || 0) +
                         (stats.alerts.overdueVaccinations.length || 0) +
                         (stats.alerts.waterQualityAlerts.length || 0)}
                      </Badge>
                  ...
                  <CardContent className="space-y-2">
                    {stats.alerts.highMortality.map((alert: MortalityAlert) => (
                      <div key={alert.batchId} className="flex items-center justify-between p-2.5 rounded-lg bg-background/80 text-sm">
                        <div className="flex items-center gap-2 min-w-0">
                          <TrendingDown className="h-4 w-4 text-red-500 shrink-0" />
                          <span className="truncate">{alert.species} - {alert.rate}% mortality</span>
                        </div>
                        <Badge variant="destructive" className="text-xs shrink-0 ml-2">{alert.rate}%</Badge>
                      </div>
                    ))}
                    {stats.alerts.overdueVaccinations.map((alert: VaccinationAlert, i: number) => (
                      <div key={i} className="flex items-center justify-between p-2.5 rounded-lg bg-background/80 text-sm">
                        <div className="flex items-center gap-2 min-w-0">
                          <Syringe className="h-4 w-4 text-amber-600 shrink-0" />
                          <span className="truncate">{alert.vaccineName} - {alert.species}</span>
                        </div>
                        <Badge variant="secondary" className="text-xs bg-amber-100 text-amber-700 shrink-0 ml-2">Overdue</Badge>
                      </div>
                    ))}
                    {stats.alerts.waterQualityAlerts.map((alert: WaterQualityAlert, i: number) => (
                      <div key={i} className="flex items-center justify-between p-2.5 rounded-lg bg-background/80 text-sm">
                        <div className="flex items-center gap-2 min-w-0">
                          ...
                          <span className="truncate">{alert.parameter}: {alert.value}</span>
                        </div>
                        <Badge variant="outline" className="text-xs shrink-0 ml-2">Check</Badge>
                      </div>
                    ))}
                  </CardContent>
                </Card>
              ...
              <Card>
                ...
                <CardContent className="space-y-2">
                    ...
                    <div key={tx.id} className="flex items-center justify-between p-2.5 rounded-lg hover:bg-muted/50 transition-colors">
                      <div className="flex items-center gap-3 min-w-0">
                        <div className={cn(
                          ...
                          {tx.type === 'sale' ? <DollarSign className="h-4 w-4" /> : <Receipt className="h-4 w-4" />}
                        </div>
                        <div className="min-w-0">
                  ...
                  {stats.recentTransactions.length === 0 && (
                    <p className="text-center py-4 text-sm text-muted-foreground">No transactions yet</p>
                  )}
                </CardContent>
              </Card>
            </div>
              ...
              <Card>
                ...
                <CardContent className="space-y-2">
                  ...
                  {stats.topCustomers.length === 0 && (
                    <p className="text-center py-4 text-sm text-muted-foreground">No customer data yet</p>
                  )}
                </CardContent>
              </Card>
              ...
              <Card>
                ...
                <CardContent>
                  <div className="relative pl-4 space-y-3 before:absolute before:left-1.5 before:top-2 before:bottom-2 before:w-0.5 before:bg-border">
                    {[
                      { icon: Plus, title: 'Batch added', desc: 'New livestock', time: '2h ago', color: 'text-emerald-600 bg-emerald-100' },
                      { icon: ShoppingCart, title: 'Sale recorded', desc: 'Customer purchase', time: '4h ago', color: 'text-blue-600 bg-blue-100' },
                      { icon: Receipt, title: 'Expense logged', desc: 'Farm supplies', time: 'Yesterday', color: 'text-red-600 bg-red-100' },
                      { icon: TrendingDown, title: 'Mortality', desc: 'Health check', time: 'Yesterday', color: 'text-amber-600 bg-amber-100' },
                    ].map((item, i) => (
                      <div key={i} className="relative">
                        <div className={cn("absolute -left-[21px] top-0 h-7 w-7 rounded-full flex items-center justify-center", item.color.replace('text-', 'bg-').replace('600', '200'))}>
